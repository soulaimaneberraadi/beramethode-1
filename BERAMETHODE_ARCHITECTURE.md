...
### 2.11 حدّ الأجهزة لكل مستخدم (قرار 2026-08-30)

المستخدم عندو **بروفيل واحد** كيسجّل بيه الدخول من عدد محدود ديال الأجهزة (الافتراضي: جهازين — تيليفون + حاسوب). ماشي بروفيل لكل جهاز.

- كل جهاز كياخد علامة تُخزَّن مع الجلسة.
- وصل للحدّ → إمّا نمنع الجهاز الجديد، ولا نُسقِط أقدم جلسة (الاختيار للأدمين).
- الحدّ قابل للضبط لكل دور.

**فائدة ثانية غير التنظيم:** كتمنع مشاركة الحسابات — بلاها شركة تخلّص على 5 مقاعد وتخدم بيهم 20.

### 2.12 تقارير الأخطاء تصل للمالك (قرار 2026-08-30)

**الوضع قبل:** `ErrorBoundary` كيصيفط تقرير أوتوماتيكي لـ `/api/errors/report` → كيتسجّل في جدول `crash_reports` **المحلي** ويبقى تما. المالك عمرو ما يشوفو. الشكايات اليدوية وحدها (زرّ «الإبلاغ») هي اللي كتوصل عبر `support_tickets`.

**القرار:** التقرير الأوتوماتيكي كيتصيفط هو تاني، بثلاث شروط:
1. **طابور محلي** — البرنامج يقدر يكون بلا إنترنت. التقرير كيتحط في طابور وكيتصيفط ملي يرجع الاتصال.
2. **تنقية إجبارية** — نصيفطو **آش طرا** (نوع الخطأ، فين، أي نسخة) ماشي **الداتا**. رسالة الخطأ ساعات فيها اسم كليان ولا ثمن.
3. **منع التكرار** — نفس الخطأ في حلقة ما يصيفطش 100 تقرير.

**بلا تعديل في مخطّط السحابة:** كنعيدو استعمال `support_tickets` بنفس الأنواع الموجودة، والتمييز عبر `context.auto = true`.

### 2.13 توزيع التحديثات عبر المنصّات (قرار 2026-08-30)

**GitHub Releases كيحمل الملفات · Bera-master-admin كيقرّر السياسة.**

ماشي Bera-master-admin كتستضيف الملفات: الملفات كبار (عشرات الميغا)، ولوحة التحكّم ماشي مستودع.

| المنصّة | كيفاش كيوصل التحديث |
|---|---|
| Windows / Mac | GitHub Releases + electron-updater — بموافقة الأدمين |
| Android / iOS | **عبر المتاجر إجبارياً** — ماشي بيد المالك |

**نتيجة مهمّة:** الآيفون كيتأخّر (مراجعة آبل تاخد أيام). يعني السيرفر والزبون خاصهم يقبلو يكونو على **نسختين مختلفتين** بلا ما يحبسو → قراءة متسامحة (2.10) ماشي اختيارية، هي شرط.

**دور Bera-master-admin:** شكون على أي نسخة · «النسخة الدنيا المقبولة» · مين ياخد التحديث ووقتاش.
