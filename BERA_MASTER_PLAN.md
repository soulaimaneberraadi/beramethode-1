...
## SECTION 24 — BERA ECOSYSTEM — الرؤية الكاملة للمنظومة

> **تاريخ التوثيق:** 2026-05-26
> **المصدر:** جلسة تصميم معمّقة مع سليمان بالعربية الفصحى
> **الحالة:** رؤية معتمدة نهائياً — مرجع لكل البرامج القادمة

---

### 24.1 — البرامج الخمسة في المنظومة

| البرنامج | يخدم مَن | الحالة |
|---------|---------|--------|
| **BERAMETHODE** | المعمل (المدير، رئيس السلسلة، أمين المخزن) | ✅ V1 قيد التطوير |
| **BERAOUVIER** | العامل الفردي | 🔴 V2 مخطط |
| **Client Portal** | زبائن المعمل | 🔴 V2 مخطط |
| **Fournisseur Portal** | موردو المعمل | 🔴 V2 مخطط |
| **BERA ADMIN** | وكالة سليمان (المالك والمشغّل) | 🔴 V2 مخطط |

---

### 24.2 — المخطط المعماري الكامل
...
```
                    ...
                    │      (وكالة سليمان)               │
                    │                                  │
                    │  • يرى بيانات أي معمل في أي وقت  │
                    │  • يوافق على كل الطلبات الواردة  │
                    │  • يقترح بالذكاء الاصطناعي       │
                    │  • يُدير التراخيص والاشتراكات    │
                    └──────────────┬───────────────────┘
                                   │
           ┌───────────────────────┼───────────────────────┐
           │                       │                       │
           ↓                       ↓                       ↓
┌──────────────────┐   ┌──────────────────┐   ┌──────────────────┐
│  🏭 BERAMETHODE  │   │ 🌐 Client Portal  │   │🔧 Fournisseur    │
│  (المعمل)        │◄──►│  (الزبون)        │   │   Portal         │
│                  │   │                  │   │  (المورّد)        │
└────────┬─────────┘   └──────────────────┘   └────────┬─────────┘
         │                                             │
         │◄────── تصفح الكتالوج + الطلب المباشر ───────┘
         │
         ↓
┌──────────────────┐
│  📱 BERAOUVIER   │
...
### 24.3 — BERA ADMIN — التعريف والصلاحيات

**التعريف:** المتحكم المركزي الأعلى في المنظومة كاملة — يملكه ويديره صاحب الوكالة (سليمان).

**الصلاحيات الجوهرية:**
```
👁️  يرى كل شيء
─────────────────────────────────
• بيانات أي معمل في أي وقت
• كل مستخدم في أي برنامج
• في الوقت الفعلي أينما كانوا

✅  يُقرّر كل شيء
─────────────────────────────────
• يُفعّل أو يُوقف أي معمل
• يوافق أو يرفض أي طلب وارد
• يمنح أو يسحب أي صلاحية
```
...
| الصفحة | الدور |
|--------|------|
| لوحة القيادة | إحصائيات كل المعامل + تنبيهات فورية |
| المعامل | قائمة كل معمل + تفعيل/إيقاف + مراقبة |
| الطلبات والموافقات | كل الطلبات الواردة → ✅ موافقة أو ❌ رفض |
| التراخيص | توليد مفاتيح + تفعيل + تجديد |
| مشاهدة البيانات | ادخل على أي معمل وشاهد كل بياناته (قراءة فقط) |
| الذكاء الاصطناعي | AI Matching (مورّد / معمل / عامل) |
| الإعدادات | إعدادات الوكالة + الأمان |

---

### 24.4 — BERAMETHODE — صفحات V1 الكاملة

| الصفحة | الحالة |
|--------|--------|
| لوحة القيادة | ✅ Done |
| الهندسة (FT + Gamme + Équilibrage + Implantation + Coûts) | ✅ Done |
| التخطيط | ✅ Done |
| المخزن | ✅ Done |
| المتابعة (Suivi Production) | ✅ Done |
| الموارد البشرية | ✅ Done |
| الفوترة | ✅ Done |
| الإدارة (Admin Dashboard) | ✅ Done |
| الإعدادات | ✅ Done |
| 🛒 تصفح موردين (داخل المخزن) | 🔴 V2 — مرتبط بـ Fournisseur Portal |

---

### 24.5 — Fournisseur Portal — نموذج Alibaba للنسيج

**الرؤية:** بوابة إلكترونية كـ Alibaba مخصصة للنسيج المغربي — المورّد ينشر بضاعته، والمعمل يطلب مباشرة من داخل BERAMETHODE.

**تدفق العمل:**
```
المورّد ينشر كتالوجه في Fournisseur Portal
(اسم المنتج + الصور + السعر + الكميات + مدة التسليم)
          ↓
في BERAMETHODE (قسم المخزن) يظهر تبويب "🛒 الموردين"
أمين المخزن يتصفح ويختار ويضغط "اطلب"
          ↓
الطلب يصل للمورّد في Fournisseur Portal
المورّد يؤكد ويُرسل
          ↓
البضاعة تُسجَّل تلقائياً في مخزن المعمل
```

**صفحات Fournisseur Portal:**

| الصفحة | الدور |
|--------|------|
| تسجيل الدخول | رمز المورّد |
| كتالوجي | أضف/عدّل منتجاتي مع الصور والأسعار |
| الطلبات الواردة | طلبات المعامل → تأكيد أو رفض |
| التسليمات | تأكيد ما تم تسليمه |
| إحصائياتي | أكثر المنتجات طلباً |

**الفرق عن Alibaba العادي:**

| Alibaba | منظومة BERA |
|---------|-------------|
| عام لكل القطاعات | خاص بالنسيج المغربي فقط |
| تطبيق منفصل | مدمج في BERAMETHODE مباشرة |
| لا يعرف احتياجاتك | يعرف مخزنك وما ينقصك |
| أنت تبحث | BERA ADMIN يقترح تلقائياً |

---

### 24.6 — Client Portal — صفحات

| الصفحة | الدور |
|--------|------|
| تسجيل الدخول | رمز الزبون |
| طلباتي | قائمة الموديلات + الحالة |
| تفاصيل الطلب | التقدم + الكميات + التسليمات |
| طلب جديد | أرسل موديل جديد → BERA ADMIN يقترح أفضل معمل |

---

### 24.7 — BERAOUVIER — صفحات

| الصفحة | الدور |
|--------|------|
| تسجيل الدخول | CIN + PIN |
| ملفي الشخصي | الاسم، المنصب، المعمل |
| حضوري اليوم | الدخول، الخروج، الساعات |
| إنتاجي اليوم | القطع + نسبة الجودة |
| فرص العمل | اقتراحات BERA ADMIN إذا كان بدون عمل |

---

### 24.8 — نظام الذكاء الاصطناعي (AI Matching)

BERA ADMIN يملك بيانات كل المعامل ويستخدمها لربط الأطراف الصحيحة ببعض:

**المحور الأول — ربط المعامل بالموردين:**
```
معمل يحتاج مواد
      ↓
BERA ADMIN يحلل: الجودة المطلوبة + الكمية + الميزانية
      ↓
يقترح أفضل مورّد من Fournisseur Portal
      ↓
يُرسل الطلب للمورّد فوراً
```

**المحور الثاني — ربط العملاء بالمعامل:**
```
عميل عنده موديل (مثلاً 20,000 قطعة)
      ↓
BERA ADMIN يحلل: طاقة المعامل + الجودة + السعر + الوقت
      ↓
يقترح على العميل أفضل معمل لتنفيذ طلبه
      ↓
يُرسل العرض للعميل فوراً
```

**المحور الثالث — ربط العمال بالمعامل:**
```
عامل بدون عمل — له بروفيل في BERAOUVIER
(مهاراته + خبرته + موقعه)
      ↓
BERA ADMIN يحلل: المعامل التي تحتاج عمالاً + المهارات المطلوبة
      ↓
يقترح على العامل المعمل الأنسب له
```

---

### 24.9 — تدفق البيانات الكامل بين البرامج

```
┌─────────────────────────────────────────────────────────┐
│                                                         │
│  1. تدفق بضاعة:                                        │
│     المورّد (Fournisseur Portal)                        │
│       → ينشر كتالوجه                                   │
│       → المعمل يطلب من BERAMETHODE (Magasin)            │
│       → البضاعة تُسجَّل تلقائياً في المخزن             │
│                                                         │
│  2. تدفق الطلبات:                                       │
│     العميل (Client Portal)                              │
│       → يرسل طلب موديل جديد                            │
│       → BERA ADMIN يقترح أفضل معمل                     │
│       → الطلب يدخل في تخطيط BERAMETHODE                │
│       → العميل يتابع التقدم في Client Portal            │
│                                                         │
│  3. تدفق العمال:                                        │
│     العامل (BERAOUVIER)                                 │
│       → يسجّل حضوره وإنتاجه                            │
│       → البيانات تُحفَظ في BERAMETHODE                  │
│       → BERA ADMIN يراها ويقترح فرص عمل                │
│                                                         │
│  4. تدفق المراقبة:                                      │
│     BERA ADMIN                                          │
│       → يرى كل شيء في الوقت الفعلي                     │
│       → يوافق على الطلبات                              │
│       → يُفعّل/يُوقف التراخيص                          │
│                                                         │
└─────────────────────────────────────────────────────────┘
```
...
### 24.10 — ترتيب بناء المنظومة (الأولويات)

```
المرحلة الحالية  → BERAMETHODE V1 (إكمال الـ phases 7-11)
                         ↓
المرحلة الثانية  → BERAOUVIER (repo منفصل، Phase 9)
                         ↓
المرحلة الثالثة  → Fournisseur Portal (مدمج مع Magasin)
                         ↓
المرحلة الرابعة  → Client Portal
                         ↓
المرحلة الخامسة  → BERA ADMIN (يتطلب وجود كل البرامج)
```

---

### 24.11 — القرارات المعتمدة (من جلسة 2026-05-26)

| # | القرار |
|---|--------|
| D20 | BERA ADMIN = BERA MASTER — نفس البرنامج، اسمان مقبولان |
| D21 | BERA ADMIN يُدار من وكالة سليمان (ليس من AJANIF TEX) |
| D22 | Fournisseur Portal يعمل كـ Alibaba للنسيج — كتالوج + طلب مباشر |
| D23 | الطلب من Fournisseur Portal يُسجَّل تلقائياً في مخزن BERAMETHODE |
| D24 | BERA ADMIN يملك نظام AI Matching لربط المعامل/الموردين/العملاء/العمال |
| D25 | BERAOUVIER يعرض فرص عمل مقترحة من BERA ADMIN للعمال بدون عمل |

---

*نهاية القسم 24 — آخر تحديث: 2026-05-26*

---

## SECTION 25 — دراسة السوق | Market Study

> **تاريخ البحث:** 2026-05-26
> **المصدر:** بيانات حقيقية من AMITH، WTO، DataIntelo، Hespress، GlobeNewswire وغيرها
> **التركيز:** المغرب أولاً ثم التوسع

---

### 25.1 — حجم السوق المغربي

| المؤشر | الرقم |
|--------|-------|
| عدد معامل النسيج في المغرب | **1,628 معمل** |
| عدد العمال المباشرين | **+200,000 عامل** |
| مساهمة القطاع في الصادرات | **11% من إجمالي صادرات المغرب** |
| حصة القطاع من الناتج الصناعي | **15% من الناتج الصناعي** |
| رقم الأعمال السنوي (2024) | **~45 مليار درهم** |
| معدل النمو السنوي | **7%** |
| صادرات النسيج للاتحاد الأوروبي | **€2.9 مليار** |
| مرتبة المغرب في تصدير النسيج للاتحاد الأوروبي | **المرتبة 8 عالمياً** |

---

### 25.2 — المناطق الجغرافية المستهدفة

| المنطقة | الأهمية |
|---------|---------|
| **طنجة-تطوان** | الأكبر — ميناء طنجة Med قريب من أوروبا — أسرع تخليص جمركي |
| **فاس-مكناس** | عنقود صناعي ضخم + استثمارات صينية جديدة ($154M) |
| **الدار البيضاء-سطات** | المركز التجاري والصناعي الأول — مشروع eco-park 100 هكتار |

---

### 25.3 — العميل المثالي (Ideal Customer Profile)

| المعيار | الملف |
|---------|-------|
| حجم المعمل | **50 → 500 عامل** (المنطقة الذهبية: 100-300) |
| رقم الأعمال السنوي | **5M → 100M درهم** |
| نوع المعمل | CMT أو Full Package — يُصدّر للأوروبيين (Zara, H&M, Decathlon) |
| الأدوات الحالية | Excel + أوراق يدوية + لا نظام متكامل |
| المشكلة الجوهرية | لا يعرف تكلفته الحقيقية، أخطاء الرواتب، لا متابعة إنتاج |

---

### 25.4 — متخذو القرار في المعمل

| الشخص | دوره في القرار | ما يريده |
|-------|---------------|---------|
| **المدير العام (PDG)** | يوافق على الشراء النهائي | الربحية + التقارير |
| **مدير الإنتاج** | مؤثر قوي | التخطيط + كفاءة الإنتاج |
| **مهندس الطرق** | المستخدم الأساسي والمدافع الداخلي | حساب الوقت + التكاليف |
| **مدير الموارد البشرية** | مستخدم يومي | التغيب + الرواتب |
| **أمين المخزن** | مستخدم يومي | الجرد + الطلبات |

---

### 25.5 — نسبة الحاجة ومعدل التبني

```
معامل النسيج في المغرب: 1,628
          ↓
تستخدم ERP حالياً: ~15-25% فقط
(أي: 240 → 400 معمل)
          ↓
75-85% لا تزال تعمل بـ Excel والأوراق
(أي: 1,200 → 1,400 معمل بدون نظام)

معدل نمو التبني الرقمي في المغرب: +12% سنة 2024-2025
```

---

### 25.6 — السوق المتاح مباشرة (TAM)

| الشريحة | عدد المعامل | متوسط العقد السنوي | الإجمالي |
|---------|-------------|-------------------|---------|
| معامل 50-500 عامل في المغرب | ~900 معمل | 30,000 → 120,000 درهم | **27M → 132M درهم/سنة** |
| بتحقيق 10% من السوق فقط | ~90 معمل | 50,000 درهم | **4.5M درهم ARR** |
| ميزانية IT المعمل (1-3% من الإيرادات) | معمل 20M درهم | 200,000 → 600,000 درهم/سنة | BERAMETHODE في المنتصف تماماً |

---

### 25.7 — تحليل المنافسين

| المنافس | السعر السنوي | المشكلة | ميزة BERAMETHODE |
|---------|-------------|---------|-----------------|
| SAP / Oracle | +500,000 درهم | مكلف جداً للمعامل الصغيرة | السعر ✅ |
| Microsoft Dynamics | +100,000 درهم | معقد + لا دعم محلي | الدعم المحلي ✅ |
| Sage (محاسبة فقط) | متوسط | ليس ERP نسيج متكامل | وحدة الطرق ✅ |
| Excel | مجاني | لا تتبع، أخطاء، لا تقارير | كل شيء ✅ |

**المزايا التنافسية الحاسمة لـ BERAMETHODE:**
1. وحدة الطرق (Prix de Revient) — غير موجودة عند أي منافس في هذا السعر
2. الفرنسية + الدارجة — الوحيد في السوق المغربي
3. التكامل مع Sage Paie المغربي
4. قانون الشغل المغربي مدمج (Article 385، ساعات إضافية)
5. دعم محلي وفهم ثقافي كامل

---

### 25.8 — العائد على الاستثمار الذي تبيعه للعميل (ROI)

```
ما يكسبه العميل بعد تطبيق BERAMETHODE:
─────────────────────────────────────────
• +20-30%  إنتاجية خلال السنة الأولى
• -15%     تكاليف الإنتاج
• -40%     نقص المواد الأولية
• +5-10%   هامش الربح الصافي
• 98%+     ثبات في الجودة
```

---

### 25.9 — خطة التسويق والمبيعات

#### القناة الأولى — البيع المباشر للمدير العام
```
تتصل مباشرة بـ PDG أو Directeur de Production
→ عرض تجريبي مجاني (Demo) في معمله على بياناته الحقيقية
→ نسخة تجريبية مدفوعة 30 يوم
→ عقد سنوي
```

#### القناة الثانية — شبكة الوكلاء (Agents)
```
وكلاء محليون في: طنجة + فاس + الدار البيضاء
→ يبيعون نيابة عنك بعمولة 15-20%
→ يوفرون الدعم الميداني
→ يعرفون المعامل والمدراء محلياً
```

#### القناة الثالثة — شركاء الصناعة (Marketers)
```
مستشارو الإنتاج + مكاتب الهندسة الصناعية
→ يوصون بـ BERAMETHODE لعملائهم
→ عمولة لكل عقد مُبرم
```

---

### 25.10 — أسلوب الوصول للـ CEO

| الأسلوب | التفاصيل |
|---------|---------|
| **AMITH** | الاتحاد المغربي لصناعات النسيج — احضر فعالياتهم وعرّف نفسك |
| **المعارض الصناعية** | Maroc in Mode، Maroc Sourcing — عرض مباشر |
| **LinkedIn** | استهداف PDG و Directeur de Production في طنجة وفاس |
| **التوصيات** | عميل راضٍ → يوصي 3 معامل أخرى تلقائياً |
| **Demo ميداني** | اذهب للمعمل بجهازك وأرهم النظام يعمل على بياناتهم الحقيقية |

---

### 25.11 — خريطة التوسع الجغرافي

```
المرحلة 1 (الآن)      → المغرب       — 1,628 معمل — الأولوية القصوى
          ↓
المرحلة 2 (سنة 1-2)   → تونس         — 1,400 معمل — نفس اللغة، نفس المشاكل
          ↓
المرحلة 3 (سنة 2-3)   → مصر          — 2,500 معمل — بعد تعريب جزئي
          ↓
المرحلة 4 (سنة 3+)    → أفريقيا جنوب الصحراء (السنغال، إثيوبيا...)
```

---

### 25.12 — الخلاصة في أرقام

```
┌─────────────────────────────────────────────────────┐
│  السوق المغربي:          1,628 معمل                 │
│  بدون ERP حالياً:        ~1,200 معمل (75%)          │
│  الهدف الأول:            100 معمل (6% من السوق)     │
│  متوسط العقد:            50,000 درهم/سنة            │
│  العائد السنوي المستهدف:  5,000,000 درهم ARR        │
│  التوسع لتونس:           +1,400 معمل إضافية         │
│  نمو السوق:              +12% سنوياً (2024-2025)    │
│  سوق ERP النسيج عالمياً: $4.37B — ينمو 8-12% سنوياً│
└─────────────────────────────────────────────────────┘
```

---

*نهاية القسم 25 — آخر تحديث: 2026-05-26*

---

*Fin du BERA_MASTER_PLAN.md — Version 4.0 DEFINITIVE*  
