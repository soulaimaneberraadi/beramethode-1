...
export default function LaCoupe({ models, setModels, onOpenInAtelier, currentModelId, setFicheData }: LaCoupeProps) {
    ...
    const saveModelToServer = useCallback(async (model: ModelData, successMessage?: string) => {
        try {
            ...
            if (!res.ok) throw new Error(tx(lang, { fr: 'Erreur de sauvegarde serveur', ar: 'Ø®Ø·Ø£ ÙÙŠ Ø­ÙØ¸ Ø§Ù„Ø®Ø§Ø¯Ù…', en: 'Server save error', es: 'Error de guardado del servidor', pt: 'Erro ao salvar no servidor', tr: 'Sunucu kaydetme hatasÄ±' }));
            if (successMessage) showToast(successMessage, 'success');
            return true;
        } catch (e) {
            console.error("Failed to save model to server:", e);
            showToast(tx(lang, { fr: 'Erreur de sauvegarde Cloud', ar: 'Ø®Ø·Ø£ ÙÙŠ Ø§Ù„Ø­ÙØ¸ Ø§Ù„Ø³Ø­Ø§Ø¨ÙŠ', en: 'Cloud save error', es: 'Error de guardado en la nube', pt: 'Erro ao salvar na nuvem', tr: 'Bulut kaydetme hatasÄ±' }), "error");
            return false;
        }
    }, [showToast]);
    ...
    const deleteModelFromServer = useCallback(async (modelId: string, successMessage?: string) => {
        try {
            ...
            if (!res.ok) throw new Error(tx(lang, { fr: 'Erreur de suppression serveur', ar: 'Ø®Ø·Ø£ ÙÙŠ Ø­Ø°Ù Ø§Ù„Ø®Ø§Ø¯Ù…', en: 'Server delete error', es: 'Error de eliminaciÃ³n del servidor', pt: 'Erro ao excluir no servidor', tr: 'Sunucu silme hatasÄ±' }));
            if (successMessage) showToast(successMessage, 'success');
            return true;
        } catch (e) {
            console.error("Failed to delete model on server:", e);
            showToast(tx(lang, { fr: 'Erreur de suppression Cloud', ar: 'Ø®Ø·Ø£ ÙÙŠ Ø§Ù„Ø­Ø°Ù Ø§Ù„Ø³Ø­Ø§Ø¨ÙŠ', en: 'Cloud delete error', es: 'Error de eliminaciÃ³n en la nube', pt: 'Erro ao excluir na nuvem', tr: 'Bulut silme hatasÄ±' }), "error");
            return false;
        }
    }, [showToast]);
    ...
    const handleExportExcel = () => {
        const headers = [
            tx(lang, { fr: 'RÃ©fÃ©rence', ar: 'Ø§Ù„Ù…Ø±Ø¬Ø¹', en: 'Reference', es: 'Referencia', pt: 'ReferÃªncia', tr: 'Referans' }),
            tx(lang, { fr: 'Nom ModÃ¨le', ar: 'Ø§Ø³Ù… Ø§Ù„Ù†Ù…ÙˆØ°Ø¬', en: 'Model Name', es: 'Nombre del Modelo', pt: 'Nome do Modelo', tr: 'Model AdÄ±' }),
            tx(lang, { fr: 'Statut', ar: 'Ø§Ù„Ø­Ø§Ù„Ø©', en: 'Status', es: 'Estado', pt: 'Status', tr: 'Durum' }),
            tx(lang, { fr: 'QuantitÃ©', ar: 'Ø§Ù„ÙƒÙ…ÙŠØ©', en: 'Quantity', es: 'Cantidad', pt: 'Quantidade', tr: 'Miktar' }),
            tx(lang, { fr: 'Longueur', ar: 'Ø§Ù„Ø·ÙˆÙ„', en: 'Length', es: 'Longitud', pt: 'Comprimento', tr: 'Uzunluk' }),
            tx(lang, { fr: 'Consommation', ar: 'Ø§Ù„Ø§Ø³ØªÙ‡Ù„Ø§Ùƒ', en: 'Consumption', es: 'Consumo', pt: 'Consumo', tr: 'TÃ¼ketim' }),
            tx(lang, { fr: 'Feuilles', ar: 'Ø§Ù„ØµÙØ§Ø¦Ø­', en: 'Sheets', es: 'Capas', pt: 'Folhas', tr: 'Tabakalar' }),
            tx(lang, { fr: 'Matelas', ar: 'Ø§Ù„Ù…ÙØ±Ø´Ø§Øª', en: 'Layers', es: 'Capas', pt: 'Esteiras', tr: 'Katmanlar' }),
        ];
        ...
        showToast(tx(lang, { fr: 'Export Excel rÃ©ussi', ar: 'ØªÙ… ØªØµØ¯ÙŠØ± Excel Ø¨Ù†Ø¬Ø§Ø­', en: 'Excel export successful', es: 'ExportaciÃ³n Excel exitosa', pt: 'ExportaÃ§Ã£o Excel bem-sucedida', tr: 'Excel dÄ±ÅŸa aktarma baÅŸarÄ±lÄ±' }), 'success');
    };
    ...
    const handleDuplicate = async (model: ModelData) => {
        ...
        const duplicate: ModelData = {
            ...
            meta_data: {
                ...
                nom_modele: `${model.meta_data?.nom_modele || 'ModÃ¨le'} (${tx(lang, { fr: 'Copie', ar: 'Ù†Ø³Ø®Ø©', en: 'Copy', es: 'Copia', pt: 'CÃ³pia', tr: 'Kopya' })})`,
        ...
        if (success) {
            ...
            showToast(tx(lang, { fr: 'Ordre dupliquÃ©', ar: 'ØªÙ… ØªÙƒØ±Ø§Ø± Ø§Ù„Ø£Ù…Ø±', en: 'Order duplicated', es: 'Orden duplicada', pt: 'Ordem duplicada', tr: 'Emir kopyalandÄ±' }), 'success');
        }
    ...
    const handleDropOnColumn = async (status: string) => {
        ...
        if (success) {
            ...
            showToast(`${tx(lang, { fr: 'Statut modifiÃ©', ar: 'ØªÙ… ØªØºÙŠÙŠØ± Ø§Ù„Ø­Ø§Ù„Ø©', en: 'Status changed', es: 'Estado modificado', pt: 'Status alterado', tr: 'Durum deÄŸiÅŸtirildi' })}: ${STATUS_MAP[status as keyof typeof STATUS_MAP]?.label}`, 'success');
        }
    ...
    const openModel = (model: ModelData) => {
        ...
        if (model.ordreCoupe) {
            ...
            setOrdre({
                refModele: model.meta_data?.nom_modele || tx(lang, { fr: 'Sans Nom', ar: 'Ø¨Ø¯ÙˆÙ† Ø§Ø³Ù…', en: 'Unnamed', es: 'Sin Nombre', pt: 'Sem Nome', tr: 'Ä°simsiz' }),
                longueurMatelas: 0,
                consommation: 0,
                nbrFeuilles: 0,
                nbrMatelas: 0,
    ...
    const handleNewModel = async () => {
        ...
        const newModel: ModelData = {
            ...
            isPublishedToLibrary: false,
            meta_data: {
                nom_modele: `${tx(lang, { fr: 'Nouveau ModÃ¨le', ar: 'Ù†Ù…ÙˆØ°Ø¬ Ø¬Ø¯ÙŠØ¯', en: 'New Model', es: 'Nuevo Modelo', pt: 'Novo Modelo', tr: 'Yeni Model' })} ${Math.floor(Math.random() * 1000)}`,
                date_creation: new Date().toISOString(),
                total_temps: 0,
                effectif: 1
            },
        ...
        if (success) {
            ...
            openModel(newModel);
            showToast(tx(lang, { fr: 'Nouveau modÃ¨le crÃ©Ã©', ar: 'ØªÙ… Ø¥Ù†Ø´Ø§Ø¡ Ù†Ù…ÙˆØ°Ø¬ Ø¬Ø¯ÙŠØ¯', en: 'New model created', es: 'Nuevo modelo creado', pt: 'Novo modelo criado', tr: 'Yeni model oluÅŸturuldu' }), 'success');
        }
    };
    ...
    const handleSaveCoupe = async (publish: boolean = false, transferTo: string | null = null) => {
                ...
                requiredMaterials.forEach((mat: any) => {
                    ...
                    if (existingIdx >= 0) {
                        ...
                        magItems[existingIdx].mouvements.push({
                            ...
                            reference: `Coupe â†’ Planning : ${ordre.refModele}`,
                            responsable: tx(lang, { fr: 'La Coupe', ar: 'Ù‚Ø³Ù… Ø§Ù„Ù‚Øµ', en: 'Cutting Dept.', es: 'Departamento de Corte', pt: 'Departamento de Corte', tr: 'Kesim DepartmanÄ±' })
                        });
                    }
                });
        ...
        const updatedModel: ModelData = {
            ...
            meta_data: {
                ...
                nom_modele: ordre.refModele || selectedModel.meta_data?.nom_modele || tx(lang, { fr: 'Sans Nom', ar: 'Ø¨Ø¯ÙˆÙ† Ø§Ø³Ù…', en: 'Unnamed', es: 'Sin Nombre', pt: 'Sem Nome', tr: 'Ä°simsiz' }),
                quantity: finalQte
            }
        };
        ...
        if (success) {
            ...
            if (transferTo) {
                ...
                showToast(`${tx(lang, { fr: 'TransfÃ©rÃ© vers', ar: 'ØªÙ… Ø§Ù„Ù†Ù‚Ù„ Ø¥Ù„Ù‰', en: 'Transferred to', es: 'Transferido a', pt: 'Transferido para', tr: 'AktarÄ±ldÄ±' })} ${transferTo}`, 'success');
            } else {
                showToast(tx(lang, { fr: 'Sauvegarde effectuÃ©e', ar: 'ØªÙ… Ø§Ù„Ø­ÙØ¸ Ø¨Ù†Ø¬Ø§Ø­', en: 'Save successful', es: 'Guardado exitoso', pt: 'Salvo com sucesso', tr: 'Kaydetme baÅŸarÄ±lÄ±' }), 'success');
            }
        }
    };
    ...
    const hexToColorName = (hex: string): string => {
        ...
        const namedColors: { name: string; r: number; g: number; b: number }[] = [
            ...
            { name: 'Vert Ã‰meraude', r: 16, g: 185, b: 129 }, { name: 'Bleu', r: 0, g: 0, b: 255 },
            ...
            { name: 'Gris Clair', r: 192, g: 192, b: 192 }, { name: 'Gris FoncÃ©', r: 64, g: 64, b: 64 },
            { name: 'Corail', r: 255, g: 127, b: 80 }, { name: 'Turquoise', r: 64, g: 224, b: 208 },
            { name: 'Kaki', r: 189, g: 183, b: 107 }, { name: 'Lavande', r: 230, g: 230, b: 250 },
            { name: 'Ivoire', r: 255, g: 255, b: 240 }, { name: 'CrÃ¨me', r: 255, g: 253, b: 208 },
        ];
    ...
    const handleDeleteOrder = async (id: string) => {
        ...
        if (success) {
            ...
            showToast(tx(lang, { fr: 'Ordre supprimÃ©', ar: 'ØªÙ… Ø­Ø°Ù Ø§Ù„Ø£Ù…Ø±', en: 'Order deleted', es: 'Orden eliminada', pt: 'Ordem excluÃ­da', tr: 'Emir silindi' }), 'success');
        }
    };

    const handleGenerateBarcodes = () => {
        showToast(tx(lang, { fr: 'Impression des Ã©tiquettes code-barres...', ar: 'Ø·Ø¨Ø§Ø¹Ø© Ù…Ù„ØµÙ‚Ø§Øª Ø§Ù„Ø¨Ø§Ø±ÙƒÙˆØ¯...', en: 'Printing barcode labels...', es: 'Imprimiendo etiquetas de cÃ³digo de barras...', pt: 'Imprimindo etiquetas de cÃ³digo de barras...', tr: 'Barkod etiketleri yazdÄ±rÄ±lÄ±yor...' }), 'info');
    };
            ...
            <div className="px-4 py-3 border-b border-slate-100 dark:border-dk-border">
                <div className="flex items-center gap-3">
                    <div className="relative flex-1">
                        ...
                        <input
                            type="text"
                            placeholder={tx(lang, { fr: 'Rechercher un ordre...', ar: 'Ø§Ù„Ø¨Ø­Ø« Ø¹Ù† Ø£Ù…Ø±...', en: 'Search an order...', es: 'Buscar una orden...', pt: 'Pesquisar uma ordem...', tr: 'Bir emir ara...' })}
                            value={searchTerm}
                ...
                <div className="flex items-center justify-between mt-2.5 px-1">
                    <div className="flex items-center gap-3">
                        <span className="text-[10px] font-semibold text-slate-500 dark:text-dk-muted">{sortedModels.length} {sortedModels.length !== 1 ? tx(lang, { fr: 'ordres', ar: 'Ø£ÙˆØ§Ù…Ø±', en: 'orders', es: 'Ã³rdenes', pt: 'ordens', tr: 'emirler' }) : tx(lang, { fr: 'ordre', ar: 'Ø£Ù…Ø±', en: 'order', es: 'orden', pt: 'ordem', tr: 'emir' })}</span>
                    ...
                    <button
                        ...
                        title={`${tx(lang, { fr: 'Trier par', ar: 'ØªØ±ØªÙŠØ¨ Ø­Ø³Ø¨', en: 'Sort by', es: 'Ordenar por', pt: 'Ordenar por', tr: 'SÄ±rala' })}: ${sortBy === 'date' ? tx(lang, { fr: 'Date', ar: 'Ø§Ù„ØªØ§Ø±ÙŠØ®', en: 'Date', es: 'Fecha', pt: 'Data', tr: 'Tarih' }) : sortBy === 'name' ? tx(lang, { fr: 'Nom', ar: 'Ø§Ù„Ø§Ø³Ù…', en: 'Name', es: 'Nombre', pt: 'Nome', tr: 'Ad' }) : sortBy === 'status' ? tx(lang, { fr: 'Statut', ar: 'Ø§Ù„Ø­Ø§Ù„Ø©', en: 'Status', es: 'Estado', pt: 'Status', tr: 'Durum' }) : tx(lang, { fr: 'QuantitÃ©', ar: 'Ø§Ù„ÙƒÙ…ÙŠØ©', en: 'Quantity', es: 'Cantidad', pt: 'Quantidade', tr: 'Miktar' })}`}
                    >
                        <ArrowUpDown className="w-3 h-3" />
                        {sortBy === 'date' ? tx(lang, { fr: 'Date', ar: 'Ø§Ù„ØªØ§Ø±ÙŠØ®', en: 'Date', es: 'Fecha', pt: 'Data', tr: 'Tarih' }) : sortBy === 'name' ? tx(lang, { fr: 'Nom', ar: 'Ø§Ù„Ø§Ø³Ù…', en: 'Name', es: 'Nombre', pt: 'Nome', tr: 'Ad' }) : sortBy === 'status' ? tx(lang, { fr: 'Statut', ar: 'Ø§Ù„Ø­Ø§Ù„Ø©', en: 'Status', es: 'Estado', pt: 'Status', tr: 'Durum' }) : tx(lang, { fr: 'QtÃ©', ar: 'Ø§Ù„ÙƒÙ…ÙŠØ©', en: 'Qty', es: 'Cant', pt: 'Qtd', tr: 'Mik' })}
                    </button>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-2 py-2 space-y-0.5">
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        {model.isPublishedToLibrary === false && (
                                            <span className="shrink-0 px-1 py-0.5 text-[8px] font-bold uppercase text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/30 border border-amber-200 rounded">{tx(lang, { fr: 'Draft', ar: 'Ù…Ø³ÙˆØ¯Ø©', en: 'Draft', es: 'Borrador', pt: 'Rascunho', tr: 'Taslak' })}</span>
                                        )}
                                    </div>
                ...
                {filteredModels.length === 0 && (
                    <div className="text-center py-16 px-4">
                        ...
                        <p className="text-[13px] font-semibold text-slate-600 dark:text-dk-text-soft">{tx(lang, { fr: 'Aucun ordre trouvÃ©', ar: 'Ù„Ù… ÙŠØªÙ… Ø§Ù„Ø¹Ø«ÙˆØ± Ø¹Ù„Ù‰ Ø£ÙŠ Ø£Ù…Ø±', en: 'No order found', es: 'Ninguna orden encontrada', pt: 'Nenhuma ordem encontrada', tr: 'HiÃ§bir emir bulunamadÄ±' })}</p>
                        <p className="text-[11px] text-slate-400 dark:text-dk-muted mt-1.5 max-w-[200px] mx-auto">
                            {searchTerm ? tx(lang, { fr: 'Essayez un autre terme de recherche', ar: 'Ø¬Ø±Ø¨ Ù…ØµØ·Ù„Ø­ Ø¨Ø­Ø« Ø¢Ø®Ø±', en: 'Try another search term', es: 'Pruebe otro tÃ©rmino de bÃºsqueda', pt: 'Tente outro termo de pesquisa', tr: 'BaÅŸka bir arama terimi deneyin' }) : tx(lang, { fr: 'CrÃ©ez un nouvel ordre pour commencer', ar: 'Ø£Ù†Ø´Ø¦ Ø£Ù…Ø±Ù‹Ø§ Ø¬Ø¯ÙŠØ¯Ù‹Ø§ Ù„Ù„Ø¨Ø¯Ø¡', en: 'Create a new order to start', es: 'Cree una nueva orden para empezar', pt: 'Crie uma nova ordem para comeÃ§ar', tr: 'BaÅŸlamak iÃ§in yeni bir emir oluÅŸturun' })}
                        </p>
                        {!searchTerm && (
                            <button
                            ...
                            >
                                <Plus className="w-3 h-3" strokeWidth={2.5} /> {tx(lang, { fr: 'Nouvel Ordre', ar: 'Ø£Ù…Ø± Ø¬Ø¯ÙŠØ¯', en: 'New Order', es: 'Nueva Orden', pt: 'Nova Ordem', tr: 'Yeni Emir' })}
                            </button>
                        )}
                    </div>
                )}
    ...
    return (
        <div className="h-full flex flex-col bg-slate-50 dark:bg-dk-bg select-none text-slate-800 dark:text-dk-text antialiased relative">
            {/* HEADER â€” Planning-style minimal */}
            <header className="shrink-0 bg-white dark:bg-dk-surface border-b border-slate-100 dark:border-dk-border relative z-20">
                <div className={`${isMobile ? 'px-3 h-12' : 'px-6 h-14'} flex items-center gap-3`}>
                    ...
                    <div className="flex items-baseline gap-2 shrink-0">
                        <h1 className={`${isMobile ? 'text-[15px]' : 'text-[15px]'} font-semibold text-slate-900 dark:text-dk-text tracking-tight`}>{tx(lang, { fr: 'La Coupe', ar: 'Ù‚Ø³Ù… Ø§Ù„Ù‚Øµ', en: 'Cutting Dept.', es: 'Departamento de Corte', pt: 'Corte', tr: 'Kesim' })}</h1>
                        {!isMobile && <span className="text-[12px] text-slate-400 dark:text-dk-muted">{tx(lang, { fr: 'Ordre de Fabrication', ar: 'Ø£Ù…Ø± ØªØµÙ†ÙŠØ¹', en: 'Production Order', es: 'Orden de FabricaciÃ³n', pt: 'Ordem de FabricaÃ§Ã£o', tr: 'Ãœretim Emri' })}</span>}
                    </div>

                    {/* Stats inline */}
                    {!isMobile && (
                        <div className="hidden md:flex items-center gap-4 ml-2">
                            <HeaderStat label={tx(lang, { fr: 'Total', ar: 'Ø§Ù„Ù…Ø¬Ù…ÙˆØ¹', en: 'Total', es: 'Total', pt: 'Total', tr: 'Toplam' })} value={coupeModels.length} />
                            <HeaderStat label={tx(lang, { fr: 'PrÃ©paration', ar: 'ØªØ­Ø¶ÙŠØ±', en: 'Preparation', es: 'PreparaciÃ³n', pt: 'PreparaÃ§Ã£o', tr: 'HazÄ±rlÄ±k' })} value={prepCount} color="bg-slate-400" />
                            <HeaderStat label={tx(lang, { fr: 'En cours', ar: 'Ù‚ÙŠØ¯ Ø§Ù„ØªÙ†ÙÙŠØ°', en: 'In Progress', es: 'En Curso', pt: 'Em Andamento', tr: 'Devam Ediyor' })} value={activeCount} color="bg-blue-500" />
                            <HeaderStat label={tx(lang, { fr: 'ValidÃ©s', ar: 'ØªÙ… Ø§Ù„ØªØ­Ù‚Ù‚', en: 'Validated', es: 'Validados', pt: 'Validados', tr: 'Onaylanan' })} value={valCount} color="bg-emerald-500" />
                        </div>
                    )}
                    ...
                    {!isMobile && (
                        <div className="hidden lg:flex items-center bg-slate-100 dark:bg-dk-elevated rounded-lg p-0.5 gap-0.5 ml-2">
                            {[
                                { id: 'list', icon: List, label: tx(lang, { fr: 'Liste', ar: 'Ù‚Ø§Ø¦Ù…Ø©', en: 'List', es: 'Lista', pt: 'Lista', tr: 'Liste' }) },
                                { id: 'board', icon: LayoutGrid, label: tx(lang, { fr: 'Tableau', ar: 'Ù„ÙˆØ­Ø©', en: 'Board', es: 'Tablero', pt: 'Quadro', tr: 'Pano' }) },
                                { id: 'calendar', icon: Calendar, label: tx(lang, { fr: 'Calendrier', ar: 'ØªÙ‚ÙˆÙŠÙ…', en: 'Calendar', es: 'Calendario', pt: 'CalendÃ¡rio', tr: 'Takvim' }) },
                                { id: 'stats', icon: BarChart3, label: tx(lang, { fr: 'Stats', ar: 'Ø¥Ø­ØµØ§Ø¦ÙŠØ§Øª', en: 'Stats', es: 'EstadÃ­sticas', pt: 'EstatÃ­sticas', tr: 'Ä°statistikler' }) },
                            ].map(v => {
                                const Icon = v.icon;
                                return (
                                    <button
                                        ...
                                        className={`flex items-center gap-1 px-2.5 py-1.5 rounded-md text-[10px] font-semibold transition-all ${
                                            viewMode === v.id
                                                ? 'bg-white dark:bg-dk-surface text-slate-900 dark:text-dk-text shadow-sm dark:shadow-dk-sm'
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                        {selectedModel && !isMobile && (
                            <button
                                ...
                                title={tx(lang, { fr: 'Retour Ã  la liste', ar: 'Ø§Ù„Ø¹ÙˆØ¯Ø© Ø¥Ù„Ù‰ Ø§Ù„Ù‚Ø§Ø¦Ù…Ø©', en: 'Back to list', es: 'Volver a la lista', pt: 'Voltar Ã  lista', tr: 'Listeye dÃ¶n' })}
                            >
                                <ArrowRight className="w-4 h-4" strokeWidth={2} />
                                <span>{tx(lang, { fr: 'Retour', ar: 'Ø¹ÙˆØ¯Ø©', en: 'Back', es: 'Volver', pt: 'Voltar', tr: 'Geri' })}</span>
                            </button>
                        )}
                        <button
                            ...
                            title={tx(lang, { fr: 'Filtres', ar: 'Ù…Ø±Ø´Ø­Ø§Øª', en: 'Filters', es: 'Filtros', pt: 'Filtros', tr: 'Filtreler' })}
                        >
                            <Filter className="w-4 h-4" strokeWidth={1.75} />
                            {!isMobile && <span>{tx(lang, { fr: 'Filtres', ar: 'Ù…Ø±Ø´Ø­Ø§Øª', en: 'Filters', es: 'Filtros', pt: 'Filtros', tr: 'Filtreler' })}</span>}
                        </button>
                        <button
                            ...
                            title={tx(lang, { fr: 'Export Excel', ar: 'ØªØµØ¯ÙŠØ± Excel', en: 'Export Excel', es: 'Exportar Excel', pt: 'Exportar Excel', tr: 'Excel Aktar' })}
                        >
                            <Download className="w-4 h-4" strokeWidth={1.75} />
                            {!isMobile && <span>{tx(lang, { fr: 'Export', ar: 'ØªØµØ¯ÙŠØ±', en: 'Export', es: 'Exportar', pt: 'Exportar', tr: 'Aktar' })}</span>}
                        </button>
                        {selectedModel && (
                            <>
                                <button
                                    ...
                                    title={tx(lang, { fr: 'MÃ©thodes', ar: 'Ø§Ù„Ø·Ø±Ù‚', en: 'Methods', es: 'MÃ©todos', pt: 'MÃ©todos', tr: 'YÃ¶ntemler' })}
                                >
                                    <Layers className="w-4 h-4" strokeWidth={1.75} />
                                    {!isMobile && tx(lang, { fr: 'MÃ©thodes', ar: 'Ø§Ù„Ø·Ø±Ù‚', en: 'Methods', es: 'MÃ©todos', pt: 'MÃ©todos', tr: 'YÃ¶ntemler' })}
                                </button>
                                <button
                                    ...
                                    title={tx(lang, { fr: 'Sauvegarder', ar: 'Ø­ÙØ¸', en: 'Save', es: 'Guardar', pt: 'Salvar', tr: 'Kaydet' })}
                                >
                                    <Save className="w-4 h-4" strokeWidth={2} />
                                    {!isMobile && tx(lang, { fr: 'Sauvegarder', ar: 'Ø­ÙØ¸', en: 'Save', es: 'Guardar', pt: 'Salvar', tr: 'Kaydet' })}
                                </button>
                                {selectedModel.isPublishedToLibrary === false && (
                                    <button
                                        ...
                                        title={tx(lang, { fr: 'Publier', ar: 'Ù†Ø´Ø±', en: 'Publish', es: 'Publicar', pt: 'Publicar', tr: 'YayÄ±nla' })}
                                    >
                                        <Send className="w-4 h-4" strokeWidth={2} />
                                        {!isMobile && tx(lang, { fr: 'Publier', ar: 'Ù†Ø´Ø±', en: 'Publish', es: 'Publicar', pt: 'Publicar', tr: 'YayÄ±nla' })}
                                    </button>
                                )}
                                {ordre.status === 'VALIDE' && (
                                    <button
                                        ...
                                        title={tx(lang, { fr: 'Vers Planning', ar: 'Ø¥Ù„Ù‰ Ø§Ù„ØªØ®Ø·ÙŠØ·', en: 'To Planning', es: 'A PlanificaciÃ³n', pt: 'Para Planejamento', tr: 'Planlamaya Git' })}
                                    >
                                        <Truck className="w-4 h-4" strokeWidth={2} />
                                        {!isMobile && tx(lang, { fr: 'Vers Planning', ar: 'Ø¥Ù„Ù‰ Ø§Ù„ØªØ®Ø·ÙŠØ·', en: 'To Planning', es: 'A PlanificaciÃ³n', pt: 'Para Planejamento', tr: 'Planlamaya Git' })}
                                    </button>
                                )}
                            </>
                        )}
                        ...
                        {!selectedModel && (
                            ...
                            >
                                ...
                                {!isMobile && tx(lang, { fr: 'Nouvel Ordre', ar: 'Ø£Ù…Ø± Ø¬Ø¯ÙŠØ¯', en: 'New Order', es: 'Nueva Orden', pt: 'Nova Ordem', tr: 'Yeni Emir' })}
                            </button>
                        )}

                        <button
                            onClick={() => showToast(tx(lang, { fr: 'Fonction impression Ã  venir', ar: 'ÙˆØ¸ÙŠÙØ© Ø§Ù„Ø·Ø¨Ø§Ø¹Ø© Ù‚Ø§Ø¯Ù…Ø© Ù‚Ø±ÙŠØ¨Ù‹Ø§', en: 'Print function coming soon', es: 'FunciÃ³n de impresiÃ³n prÃ³ximamente', pt: 'FunÃ§Ã£o de impressÃ£o em breve', tr: 'YazdÄ±rma iÅŸlevi Ã§ok yakÄ±nda' }), 'info')}
                            ...
                            title={tx(lang, { fr: 'Imprimer', ar: 'Ø·Ø¨Ø§Ø¹Ø©', en: 'Print', es: 'Imprimir', pt: 'Imprimir', tr: 'YazdÄ±r' })}
                        >
                            <Printer className="w-4 h-4" strokeWidth={1.75} />
                            {!isMobile && tx(lang, { fr: 'Rapport', ar: 'ØªÙ‚Ø±ÙŠØ±', en: 'Report', es: 'Informe', pt: 'RelatÃ³rio', tr: 'Rapor' })}
                        </button>
                        {selectedModel && (
                            <button
                                ...
                                title={tx(lang, { fr: 'Supprimer', ar: 'Ø­Ø°Ù', en: 'Delete', es: 'Eliminar', pt: 'Excluir', tr: 'Sil' })}
                            >
            ...
            <div className="flex-1 flex overflow-hidden">
                {/* Sidebar â€” always on desktop, drawer on mobile */}
                {!isMobile ? (
                    sidebar
                ) : (
                    <>
                ...
                <div className="flex-1 overflow-y-auto">
                    {showFilters && (
                        <div className="bg-white dark:bg-dk-surface border-b border-slate-100 dark:border-dk-border px-4 md:px-6 py-3 flex flex-wrap items-center gap-2">
                            <span className="text-[10px] font-bold text-slate-400 dark:text-dk-muted uppercase tracking-wide">{tx(lang, { fr: 'Statut', ar: 'Ø§Ù„Ø­Ø§Ù„Ø©', en: 'Status', es: 'Estado', pt: 'Status', tr: 'Durum' })}:</span>
                            {[
                                { v: 'ALL', l: tx(lang, { fr: 'Tous', ar: 'Ø§Ù„ÙƒÙ„', en: 'All', es: 'Todos', pt: 'Todos', tr: 'TÃ¼mÃ¼' }), c: 'bg-slate-100 dark:bg-dk-elevated text-slate-700 dark:text-dk-text-soft' },
                                { v: 'EN_PREPARATION', l: tx(lang, { fr: 'PrÃ©paration', ar: 'ØªØ­Ø¶ÙŠØ±', en: 'Preparation', es: 'PreparaciÃ³n', pt: 'PreparaÃ§Ã£o', tr: 'HazÄ±rlÄ±k' }), c: 'bg-slate-50 dark:bg-dk-bg text-slate-700 dark:text-dk-text-soft' },
                                { v: 'EN_COURS', l: tx(lang, { fr: 'En Cours', ar: 'Ù‚ÙŠØ¯ Ø§Ù„ØªÙ†ÙÙŠØ°', en: 'In Progress', es: 'En Curso', pt: 'Em Andamento', tr: 'Devam Ediyor' }), c: 'bg-blue-50 dark:bg-blue-900/30 text-blue-700' },
                                { v: 'SOUS_TRAITANCE', l: tx(lang, { fr: 'ExtÃ©riorisÃ©', ar: 'Ù…Ù‚Ø§ÙˆÙ„ Ø®Ø§Ø±Ø¬ÙŠ', en: 'Subcontracted', es: 'Subcontratado', pt: 'Terceirizado', tr: 'TaÅŸeron' }), c: 'bg-purple-50 dark:bg-purple-900/30 text-purple-700' },
                                { v: 'VALIDE', l: tx(lang, { fr: 'ValidÃ©', ar: 'ØªÙ… Ø§Ù„ØªØ­Ù‚Ù‚', en: 'Validated', es: 'Validado', pt: 'Validado', tr: 'OnaylandÄ±' }), c: 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700' },
                                { v: 'REJETE', l: tx(lang, { fr: 'RejetÃ©', ar: 'Ù…Ø±ÙÙˆØ¶', en: 'Rejected', es: 'Rechazado', pt: 'Rejeitado', tr: 'Reddedildi' }), c: 'bg-rose-50 dark:bg-rose-900/30 text-rose-700' },
                            ].map(f => (
                                <button
                                    key={f.v}
                                        ...
                                        <div className="flex gap-4 items-center">
                                            ...
                                            <div>
                                                ...
                                                <input
                                                    ...
                                                    placeholder={tx(lang, { fr: 'Nom de la rÃ©fÃ©rence...', ar: 'Ø§Ø³Ù… Ø§Ù„Ù…Ø±Ø¬Ø¹...', en: 'Reference name...', es: 'Nombre de la referencia...', pt: 'Nome da referÃªncia...', tr: 'Referans adÄ±...' })}
                                                />
                                                <p className="text-slate-400 dark:text-dk-muted mt-1 text-[11px] font-medium">{tx(lang, { fr: 'ParamÃ¨tres de matelassage', ar: 'Ø¥Ø¹Ø¯Ø§Ø¯Ø§Øª Ø§Ù„Ù…ÙØ±Ø´Ø©', en: 'Layering settings', es: 'ConfiguraciÃ³n de capas', pt: 'ConfiguraÃ§Ãµes de esteiramento', tr: 'Katman ayarlarÄ±' })}</p>
                                            </div>
                                        </div>
                                        <div className="bg-white dark:bg-dk-surface/5 backdrop-blur-md border border-white/10 rounded-xl p-3 text-center">
                                            <p className="text-slate-400 dark:text-dk-muted text-[9px] font-bold uppercase tracking-wider mb-1">{tx(lang, { fr: 'QuantitÃ©', ar: 'Ø§Ù„ÙƒÙ…ÙŠØ©', en: 'Quantity', es: 'Cantidad', pt: 'Quantidade', tr: 'Miktar' })}</p>
                                ...
                                <div className="p-4 md:p-6">
                                    <div className="flex items-center justify-between mb-5">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <h3 className="text-[14px] font-semibold text-slate-800 dark:text-dk-text">{tx(lang, { fr: 'ParamÃ¨tres du Matelas', ar: 'Ø¥Ø¹Ø¯Ø§Ø¯Ø§Øª Ø§Ù„Ù…ÙØ±Ø´Ø©', en: 'Layering Parameters', es: 'ParÃ¡metros de Capas', pt: 'ParÃ¢metros do Esteiramento', tr: 'Katman Parametreleri' })}</h3>
                                        </div>
                    ...
                    {Object.entries(STATUS_MAP).map(([key, config]: [string, any]) => {
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 mb-6">
                                        <div>
                                            <label className="block text-[10px] font-bold text-slate-400 dark:text-dk-muted mb-1.5 uppercase tracking-wide">
                                                {tx(lang, { fr: 'Consommation RÃ©fÃ©rence / PiÃ¨ce', ar: 'Ø§Ø³ØªÙ‡Ù„Ø§Ùƒ Ø§Ù„Ù…Ø±Ø¬Ø¹ / Ø§Ù„Ù‚Ø·Ø¹Ø©', en: 'Reference Consumption / Piece', es: 'Consumo Referencia / Pieza', pt: 'Consumo ReferÃªncia / PeÃ§a', tr: 'Referans TÃ¼ketimi / ParÃ§a' })}
                                            </label>
                                            <div className="relative">
                                                <input
                                                    ...
                                                    placeholder="0.00"
                                                />
                                                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-bold text-slate-400 dark:text-dk-muted bg-slate-200 px-1.5 py-0.5 rounded">MÃ¨tres</span>
                                            </div>
                                        </div>
                                        <div>
                                            <label className="block text-[10px] font-bold text-slate-400 dark:text-dk-muted mb-1.5 uppercase tracking-wide">
                                                {tx(lang, { fr: 'Tissu ReÃ§u (MÃ©trage Initial)', ar: 'Ø§Ù„Ù†Ø³ÙŠØ¬ Ø§Ù„Ù…Ø³ØªÙ„Ù… (Ø§Ù„Ù…ØªØ±Ø§Øª Ø§Ù„Ø£ÙˆÙ„ÙŠØ©)', en: 'Fabric Received (Initial Length)', es: 'Tejido Recibido (Metraje Inicial)', pt: 'Tecido Recebido (Metragem Inicial)', tr: 'AlÄ±nan KumaÅŸ (BaÅŸlangÄ±Ã§ MetrajÄ±)' })}
                                            </label>
                                            <div className="relative">
                                                <input
                                                    type="number"
                                    ...
                                    <div className="border-t border-slate-100 dark:border-dk-border pt-5 mb-6">
                                        <div className="flex items-center justify-between mb-4">
                                            ...
                                            >
                                                <Plus className="w-3 h-3" /> {tx(lang, { fr: 'Ajouter Matelas', ar: 'Ø¥Ø¶Ø§ÙØ© Ù…ÙØ±Ø´Ø©', en: 'Add Layer', es: 'Agregar Capa', pt: 'Adicionar Esteira', tr: 'Katman Ekle' })}
                                            </button>
                                        </div>
                                        ...
                                        {(ordre.matelasLines || []).length === 0 ? (
                                            <div className="text-center py-6 text-slate-400 dark:text-dk-muted text-[12px] font-medium bg-slate-50 dark:bg-dk-bg rounded-lg border border-dashed border-slate-200 dark:border-dk-border">
                                                {tx(lang, { fr: 'Aucun matelas dÃ©fini. Ajoutez une ligne pour commencer Ã  couper.', ar: 'Ù„Ù… ÙŠØªÙ… ØªØ­Ø¯ÙŠØ¯ Ø£ÙŠ Ù…ÙØ±Ø´Ø©. Ø£Ø¶Ù Ø®Ø·Ù‹Ø§ Ù„Ø¨Ø¯Ø¡ Ø§Ù„Ù‚Øµ.', en: 'No layer defined. Add a line to start cutting.', es: 'Ninguna capa definida. Agregue una lÃ­nea para comenzar a cortar.', pt: 'Nenhuma esteira definida. Adicione uma linha para comeÃ§ar a cortar.', tr: 'HiÃ§bir katman tanÄ±mlanmadÄ±. Kesmeye baÅŸlamak iÃ§in bir satÄ±r ekleyin.' })}
                                            </div>
                                        ) : (
                                                ...
                                                <table className="w-full text-[12px] border-collapse border border-slate-200 dark:border-dk-border bg-white dark:bg-dk-surface rounded-lg overflow-hidden min-w-[650px]">
                                                    <thead>
                                                        <tr className="bg-slate-50 dark:bg-dk-bg text-slate-600 dark:text-dk-text-soft border-b border-slate-200 dark:border-dk-border text-[10px] uppercase tracking-wider text-left">
                                                            <th className="py-2.5 px-3 font-bold text-center w-12">{tx(lang, { fr: 'NÂ°', ar: 'Ø±Ù‚Ù…', en: 'No.', es: 'NÂ°', pt: 'NÂ°', tr: 'No.' })}</th>
                                    ...
                                    {(ordre.matelasLines || []).length > 0 && (
                                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 border-t border-slate-100 dark:border-dk-border pt-5">
                                            ...
                                            <div className="bg-slate-50 dark:bg-dk-bg rounded-xl p-4 border border-slate-200 dark:border-dk-border">
                                                <h5 className="text-[11px] font-bold text-slate-500 dark:text-dk-muted uppercase tracking-wider mb-2.5">{tx(lang, { fr: 'Bilan par Taille (Cible vs RÃ©alisÃ©)', ar: 'Ø§Ù„Ø­Ø³Ø¨Ù†Ø© Ø­Ø³Ø¨ Ø§Ù„Ù…Ù‚Ø§Ø³ (Ø§Ù„Ù…Ø³ØªÙ‡Ø¯Ù Ù…Ù‚Ø§Ø¨Ù„ Ø§Ù„Ù…Ø­Ù‚Ù‚)', en: 'Balance by Size (Target vs Actual)', es: 'Balance por Talla (Objetivo vs Realizado)', pt: 'BalanÃ§o por Tamanho (Alvo vs Realizado)', tr: 'Beden BazÄ±nda BilanÃ§o (Hedef vs GerÃ§ekleÅŸen)' })}</h5>
                                                <div className="overflow-x-auto">
                                                    <table className="w-full text-[11px] border-collapse bg-white dark:bg-dk-surface rounded-lg overflow-hidden border border-slate-200 dark:border-dk-border">
                                                        <thead>
                                                            <tr className="bg-slate-100 dark:bg-dk-elevated text-slate-600 dark:text-dk-text-soft border-b border-slate-200 dark:border-dk-border text-[9px] uppercase tracking-wider text-left">
                                                                <th className="py-2 px-2.5 font-bold">{tx(lang, { fr: 'Mesure', ar: 'Ø§Ù„Ù…Ù‚Ø§Ø³', en: 'Size', es: 'Medida', pt: 'Medida', tr: 'Ã–lÃ§Ã¼' })}</th>
                                                                {sizes.map((s, idx) => (
                                                                    <th key={idx} className="py-2 px-1 text-center font-bold">{s}</th>
                                                                ))}
                                                                <th className="py-2 px-2.5 text-center font-bold bg-slate-200 text-slate-700 dark:text-dk-text-soft w-16">{tx(lang, { fr: 'Total', ar: 'Ø§Ù„Ù…Ø¬Ù…ÙˆØ¹', en: 'Total', es: 'Total', pt: 'Total', tr: 'Toplam' })}</th>
                                                            </tr>
                                                        </thead>
                                                        <tbody className="divide-y divide-slate-100 dark:divide-dk-border">
                                                            <tr>
                                                                <td className="py-2 px-2.5 font-semibold text-slate-600 dark:text-dk-text-soft">{tx(lang, { fr: 'Cible (Fiche)', ar: 'Ø§Ù„Ù…Ø³ØªÙ‡Ø¯Ù (Ø§Ù„ÙˆØ±Ù‚Ø©)', en: 'Target (Sheet)', es: 'Objetivo (Ficha)', pt: 'Alvo (Ficha)', tr: 'Hedef (FiÅŸ)' })}</td>
                                                                {sizes.map((s, idx) => {
                                                            ...
                                                            <tr>
                                                                <td className="py-2 px-2.5 font-semibold text-indigo-700 dark:text-dk-accent-text bg-indigo-50 dark:bg-indigo-900/30 dark:bg-dk-accent/20/10">{tx(lang, { fr: 'RÃ©alisÃ© (Coupe)', ar: 'Ø§Ù„Ù…Ø­Ù‚Ù‚ (Ø§Ù„Ù‚Øµ)', en: 'Actual (Cut)', es: 'Realizado (Corte)', pt: 'Realizado (Corte)', tr: 'GerÃ§ekleÅŸen (Kesim)' })}</td>
                                                                {sizes.map((s, idx) => (
                                            ...
                                            <div className="bg-slate-50 dark:bg-dk-bg rounded-xl p-4 border border-slate-200 dark:border-dk-border flex flex-col justify-between">
                                                <div>
                                                    <h5 className="text-[11px] font-bold text-slate-500 dark:text-dk-muted uppercase tracking-wider mb-2.5">{tx(lang, { fr: 'Bilan MatiÃ¨re', ar: 'Ø­Ø³Ø§Ø¨ Ø§Ù„Ù…ÙˆØ§Ø¯', en: 'Material Balance', es: 'Balance de Material', pt: 'BalanÃ§o de Material', tr: 'Malzeme BilanÃ§osu' })} (Ø§Ù„Ø«ÙˆØ¨ ÙˆØ§Ù„Ù…Ø®Ø²ÙˆÙ†)</h5>
                                                    <div className="grid grid-cols-3 gap-2 text-center">
                                                        <div className="bg-white dark:bg-dk-surface rounded-lg p-2 border border-slate-100 dark:border-dk-border">
                                                            <p className="text-[9px] font-bold text-slate-400 dark:text-dk-muted uppercase">{tx(lang, { fr: 'Tissu ReÃ§u', ar: 'Ø§Ù„Ù†Ø³ÙŠØ¬ Ø§Ù„Ù…Ø³ØªÙ„Ù…', en: 'Fabric Received', es: 'Tejido Recibido', pt: 'Tecido Recebido', tr: 'AlÄ±nan KumaÅŸ' })}</p>
                                                            <p className="text-sm font-bold text-slate-700 dark:text-dk-text-soft mt-0.5">{ordre.tissuRecu || 0} m</p>
                                                        </div>
                                                        <div className="bg-white dark:bg-dk-surface rounded-lg p-2 border border-slate-100 dark:border-dk-border">
                                                            <p className="text-[9px] font-bold text-slate-400 dark:text-dk-muted uppercase">{tx(lang, { fr: 'ConsommÃ©', ar: 'Ø§Ù„Ù…Ø³ØªÙ‡Ù„Ùƒ', en: 'Consumed', es: 'Consumido', pt: 'Consumido', tr: 'TÃ¼ketilen' })}</p>
                                                        ...
                                                        {(() => {
                                                            ...
                                                            return (
                                                                <div className={`rounded-lg p-2 border ${solde >= 0 ? 'bg-emerald-50 dark:bg-emerald-900/30/55 border-emerald-100 text-emerald-800' : 'bg-rose-50 dark:bg-rose-900/30/55 border-rose-100 text-rose-800'}`}>
                                                                    <p className="text-[9px] font-bold uppercase opacity-80">{tx(lang, { fr: 'Reste (DIF)', ar: 'Ø§Ù„Ù…ØªØ¨Ù‚ÙŠ', en: 'Remaining (DIF)', es: 'Restante (DIF)', pt: 'Restante (DIF)', tr: 'Kalan (DIF)' })}</p>
                                                        ...
                                                        if (gap >= 0) {
                                                            ...
                                                            return (
                                                                <div className="p-2.5 bg-orange-50 dark:bg-orange-900/30 rounded-lg border border-orange-200 flex gap-2">
                                                                    ...
                                                                    <div>
                                                                        <p className="text-[11px] font-bold text-orange-800">{tx(lang, { fr: 'Surconsommation', ar: 'Ø§Ø³ØªÙ‡Ù„Ø§Ùƒ Ø²Ø§Ø¦Ø¯', en: 'Overconsumption', es: 'Sobreconsumo', pt: 'Sobreconsumo', tr: 'AÅŸÄ±rÄ± TÃ¼ketim' })} ( Ø¶ÙŠØ§Ø¹ Ø§Ù„Ø«ÙˆØ¨ )</p>
                                                                        <p className="text-[10px] text-orange-700/90 mt-0.5">
                                                                            {tx(lang, { fr: 'DÃ©passement de', ar: 'ØªØ¬Ø§ÙˆØ²', en: 'Excess of', es: 'Exceso de', pt: 'Excedente de', tr: 'Fazla' })} <strong>{Math.abs(gap).toFixed(1)}m</strong> {tx(lang, { fr: 'par rapport au besoin thÃ©orique', ar: 'Ù…Ù‚Ø§Ø¨Ù„ Ø§Ù„Ø§Ø­ØªÙŠØ§Ø¬ Ø§Ù„Ù†Ø¸Ø±ÙŠ', en: 'compared to theoretical need', es: 'con respecto a la necesidad teÃ³rica', pt: 'em relaÃ§Ã£o Ã  necessidade teÃ³rica', tr: 'teorik ihtiyaca kÄ±yasla' })} ({targetNeed.toFixed(1)}m).
                                                                        </p>
                                                                    </div>
                                                                </div>
                                                            );
                            ...
                            <div className="bg-white dark:bg-dk-surface rounded-xl border border-slate-200 dark:border-dk-border overflow-hidden">
                                <div className="px-4 md:px-6 py-4 border-b border-slate-100 dark:border-dk-border flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <h3 className="text-[14px] font-semibold text-slate-800 dark:text-dk-text">{tx(lang, { fr: 'RÃ©partition Tailles / Couleurs', ar: 'ØªÙˆØ²ÙŠØ¹ Ø§Ù„Ù…Ù‚Ø§Ø³Ø§Øª / Ø§Ù„Ø£Ù„ÙˆØ§Ù†', en: 'Size / Color Distribution', es: 'DistribuciÃ³n de Tallas / Colores', pt: 'DistribuiÃ§Ã£o de Tamanhos / Cores', tr: 'Beden / Renk DaÄŸÄ±lÄ±mÄ±' })}</h3>
                                    </div>
                                    <div className="flex flex-wrap gap-2 items-center w-full sm:w-auto">
                                        <div className="flex items-center bg-slate-100 dark:bg-dk-elevated rounded-md p-0.5 border border-slate-200 dark:border-dk-border">
                                            <input
                                                type="text"
                                                placeholder={tx(lang, { fr: 'Tailles (ex: 36 38 40)', ar: 'Ù…Ù‚Ø§Ø³Ø§Øª (Ù…Ø«Ø§Ù„: 36 38 40)', en: 'Sizes (e.g. 36 38 40)', es: 'Tallas (ej: 36 38 40)', pt: 'Tamanhos (ex: 36 38 40)', tr: 'Bedenler (Ã¶rn: 36 38 40)' })}
                                            ...
                                            />
                                            <button onClick={handleAddSize} className="bg-white dark:bg-dk-surface rounded p-1 shadow-sm dark:shadow-dk-sm hover:text-indigo-600 dark:text-dk-accent-text transition-colors">
                                        ...
                                        >
                                            <Barcode className="w-3 h-3" /> {tx(lang, { fr: 'Tickets', ar: 'ØªØ°Ø§ÙƒØ±', en: 'Tickets', es: 'Tickets', pt: 'Etiquetas', tr: 'Etiketler' })}
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div className="px-4 md:px-6 py-2 border-b border-slate-100 dark:border-dk-border bg-slate-50 dark:bg-dk-bg/50 flex flex-wrap gap-2 items-center">
                                    <label className="relative flex items-center justify-center cursor-pointer shrink-0" title={tx(lang, { fr: 'Couleur', ar: 'Ù„ÙˆÙ†', en: 'Color', es: 'Color', pt: 'Cor', tr: 'Renk' })}>
                                    ...
                                    <div className="relative flex-1 min-w-[120px] flex items-center bg-white dark:bg-dk-surface border border-slate-200 dark:border-dk-border rounded-md focus-within:border-indigo-400 px-2 h-7">
                                        ...
                                        <ExcelInput
                                            ...
                                            placeholder={tx(lang, { fr: 'Nom couleur...', ar: 'Ø§Ø³Ù… Ø§Ù„Ù„ÙˆÙ†...', en: 'Color name...', es: 'Nombre del color...', pt: 'Nome da cor...', tr: 'Renk adÄ±...' })}
                                    ...
                                    >
                                        <Plus className="w-3 h-3" /> {tx(lang, { fr: 'Ajouter', ar: 'Ø¥Ø¶Ø§ÙØ©', en: 'Add', es: 'Agregar', pt: 'Adicionar', tr: 'Ekle' })}
                                    </button>
                                </div>
                                    ...
                                    <div className="p-3 md:p-4 overflow-x-auto">
                                        ...
                                        <thead>
                                            <tr className="bg-slate-50 dark:bg-dk-bg text-slate-600 dark:text-dk-text-soft border-b border-slate-200 dark:border-dk-border text-[10px] uppercase tracking-wider">
                                                <th className="py-3 px-3 text-right font-bold border-l border-slate-200 dark:border-dk-border min-w-[120px]">{tx(lang, { fr: 'Couleur \\ Taille', ar: 'Ø§Ù„Ù„ÙˆÙ† \\ Ø§Ù„Ù…Ù‚Ø§Ø³', en: 'Color \\ Size', es: 'Color \\ Talla', pt: 'Cor \\ Tamanho', tr: 'Renk \\ Beden' })}</th>
                                                {sizes.length === 0 && (
                                                    <th className="py-3 px-3 text-center font-normal italic text-slate-400 dark:text-dk-muted border-l border-slate-200 dark:border-dk-border">{tx(lang, { fr: 'Aucune taille', ar: 'Ù„Ø§ ÙŠÙˆØ¬Ø¯ Ù…Ù‚Ø§Ø³', en: 'No size', es: 'Ninguna talla', pt: 'Nenhum tamanho', tr: 'Beden yok' })}</th>
                                                )}
                                                {sizes.map((s, i) => (
                                                    <th
                                                        key={i}
                                                ...
                                                <th className="py-3 px-3 text-center font-bold bg-slate-200 text-slate-800 dark:text-dk-text w-20">{tx(lang, { fr: 'Total', ar: 'Ø§Ù„Ù…Ø¬Ù…ÙˆØ¹', en: 'Total', es: 'Total', pt: 'Total', tr: 'Toplam' })}</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-slate-100 dark:divide-dk-border">
                                            {colors.length === 0 && (
                                                <tr>
                                                    <td colSpan={sizes.length + 2} className="py-8 text-center text-slate-400 dark:text-dk-muted text-[12px] font-medium">
                                                        {tx(lang, { fr: 'Aucune couleur dÃ©finie', ar: 'Ù„Ù… ÙŠØªÙ… ØªØ­Ø¯ÙŠØ¯ Ø£ÙŠ Ù„ÙˆÙ†', en: 'No color defined', es: 'NingÃºn color definido', pt: 'Nenhuma cor definida', tr: 'Renk tanÄ±mlanmadÄ±' })}
                                                    </td>
                                                </tr>
                                            )}
                                            {colors.map((c, cIdx) => {
                                                            ...
                                                            {editingMatrixItem?.type === 'color' && editingMatrixItem.index === cIdx ? (
                                                                ...
                                                                <div className="flex items-center gap-1.5">
                                                                    ...
                                                                    <span className="truncate max-w-[100px] text-[12px]">
                                                                        {cHex && (cName.includes('personnalisÃ©') || cName.startsWith('#') || cName.includes('rgb(')) ? hexToColorName(cHex) : cName}
                                                                    </span>
                                                                </div>
                                                            )}
                                                        </td>
                                        ...
                                        <tfoot className="border-t border-slate-200 dark:border-dk-border bg-slate-50 dark:bg-dk-bg">
                                            <tr>
                                                <td className="py-3 px-3 text-left font-bold text-slate-600 dark:text-dk-text-soft border-l border-slate-200 dark:border-dk-border text-[11px]">{tx(lang, { fr: 'GÃ‰NÃ‰RAL', ar: 'Ø§Ù„Ø¥Ø¬Ù…Ø§Ù„ÙŠ', en: 'TOTAL', es: 'GENERAL', pt: 'GERAL', tr: 'GENEL' })}</td>
                                ...
                                {matrixCtx && createPortal(
                                    <div
                                        className="fixed bg-white dark:bg-dk-surface rounded-lg shadow-xl dark:shadow-dk-elevated border border-slate-200 dark:border-dk-border w-48 z-[9999] py-1.5 text-[12px] text-slate-700 dark:text-dk-text-soft font-medium"
                                    ...
                                    >
                                        <div className="px-3 py-1 text-[9px] font-bold text-slate-400 dark:text-dk-muted uppercase tracking-wider">
                                            {matrixCtx.type === 'size' ? tx(lang, { fr: 'Taille', ar: 'Ù…Ù‚Ø§Ø³', en: 'Size', es: 'Talla', pt: 'Tamanho', tr: 'Beden' }) : tx(lang, { fr: 'Couleur', ar: 'Ù„ÙˆÙ†', en: 'Color', es: 'Color', pt: 'Cor', tr: 'Renk' })}: {matrixCtx.name}
                                        </div>
                                        ...
                                        >
                                            {tx(lang, { fr: 'Renommer', ar: 'Ø¥Ø¹Ø§Ø¯Ø© ØªØ³Ù…ÙŠØ©', en: 'Rename', es: 'Renombrar', pt: 'Renomear', tr: 'Yeniden AdlandÄ±r' })}
                                        </button>
                                        <button
                                            type="button"
                                            onClick={() => {
                                                if (matrixCtx.type === 'size') { if (confirm(`${tx(lang, { fr: 'Supprimer la taille', ar: 'Ø­Ø°Ù Ø§Ù„Ù…Ù‚Ø§Ø³', en: 'Delete size', es: 'Eliminar talla', pt: 'Excluir tamanho', tr: 'Bedeni sil' })} "${matrixCtx.name}" ?`)) removeSize(matrixCtx.index); }
                            ...
                            <div className="bg-white dark:bg-dk-surface rounded-xl border border-slate-200 dark:border-dk-border overflow-hidden">
                                <div className="px-4 md:px-6 py-4 border-b border-slate-100 dark:border-dk-border flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <h3 className="text-[14px] font-semibold text-slate-800 dark:text-dk-text">{tx(lang, { fr: 'Simulation Fournitures', ar: 'Ù…Ø­Ø§ÙƒØ§Ø© Ø§Ù„Ù…ÙˆØ§Ø¯', en: 'Supplies Simulation', es: 'SimulaciÃ³n de Suministros', pt: 'SimulaÃ§Ã£o de Suprimentos', tr: 'Malzeme SimÃ¼lasyonu' })}</h3>
                                    </div>
                                    <span className="text-[11px] font-medium text-slate-500 dark:text-dk-muted bg-slate-50 dark:bg-dk-bg px-2 py-1 rounded border border-slate-200 dark:border-dk-border">
                                        {ordre.qteTotale} {tx(lang, { fr: 'piÃ¨ces', ar: 'Ù‚Ø·Ø¹Ø©', en: 'pcs', es: 'pzas', pt: 'peÃ§as', tr: 'adet' })}
                                    </span>
                                </div>
                                ...
                                <div className="p-3 md:p-4 overflow-x-auto">
                                    {requiredMaterials.length === 0 ? (
                                        <div className="text-center py-8 text-slate-400 dark:text-dk-muted text-[12px] font-medium bg-slate-50 dark:bg-dk-bg rounded-lg border border-dashed border-slate-200 dark:border-dk-border">
                                            {tx(lang, { fr: 'Aucune fourniture dÃ©finie dans la Fiche de CoÃ»t', ar: 'Ù„Ù… ÙŠØªÙ… ØªØ­Ø¯ÙŠØ¯ Ø£ÙŠ Ù…ÙˆØ§Ø¯ ÙÙŠ Ø¨Ø·Ø§Ù‚Ø© Ø§Ù„ØªÙƒÙ„ÙØ©', en: 'No supplies defined in the Cost Sheet', es: 'NingÃºn suministro definido en la Ficha de Costo', pt: 'Nenhum suprimento definido na Ficha de Custo', tr: 'Maliyet FiÅŸinde malzeme tanÄ±mlanmadÄ±' })}
                                        </div>
                                    ) : (
                                        <table className="w-full text-[12px] border-collapse rounded-lg overflow-hidden border border-slate-200 dark:border-dk-border bg-white dark:bg-dk-surface">
                                            <thead>
                                                <tr className="bg-slate-50 dark:bg-dk-bg text-slate-600 dark:text-dk-text-soft border-b border-slate-200 dark:border-dk-border text-[10px] uppercase tracking-wider text-left">
                                                    <th className="py-3 px-3 font-bold">{tx(lang, { fr: 'Article', ar: 'Ø§Ù„Ø®Ø§Ù…Ø©', en: 'Article', es: 'ArtÃ­culo', pt: 'Artigo', tr: 'Malzeme' })}</th>
                                                    <th className="py-3 px-3 font-bold">{tx(lang, { fr: 'Besoin', ar: 'Ø§Ù„Ø§Ø­ØªÙŠØ§Ø¬', en: 'Need', es: 'Necesidad', pt: 'Necessidade', tr: 'Ä°htiyaÃ§' })}</th>
                                                    <th className="py-3 px-3 font-bold">{tx(lang, { fr: 'Stock', ar: 'Ø§Ù„Ù…Ø®Ø²ÙˆÙ†', en: 'Stock', es: 'Stock', pt: 'Estoque', tr: 'Stok' })}</th>
                                                    <th className="py-3 px-3 font-bold text-center w-24">{tx(lang, { fr: 'Statut', ar: 'Ø§Ù„Ø­Ø§Ù„Ø©', en: 'Status', es: 'Estado', pt: 'Status', tr: 'Durum' })}</th>
                                                </tr>
                                            </thead>
                                            <tbody className="divide-y divide-slate-100 dark:divide-dk-border">
                                                        ...
                                                        <td className="py-3 px-3 text-center">
                                                            {mat.isSufficient ? (
                                                                <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 rounded text-[10px] font-bold border border-emerald-200">
                                                                    <CheckCircle2 className="w-3 h-3" /> {tx(lang, { fr: 'OK', ar: 'Ù…ØªÙˆÙØ±', en: 'OK', es: 'OK', pt: 'OK', tr: 'Tamam' })}
                                                                 </span>
                                                             ) : (
                                                                 <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-rose-50 dark:bg-rose-900/30 text-rose-700 rounded text-[10px] font-bold border border-rose-200">
                                                                     <AlertCircle className="w-3 h-3" /> {tx(lang, { fr: 'Rupture', ar: 'Ù†ÙØ§Ø¯', en: 'Out of Stock', es: 'Agotado', pt: 'Esgotado', tr: 'Stok Yok' })}
                                                                </span>
                                                            )}
                                                        </td>
                                                    </tr>
                            ...
                            {!selectedModel.isPublishedToLibrary && (
                                <div className="flex justify-end pb-4">
                                    ...
                                    >
                                        ...
                                        {tx(lang, { fr: 'Valider & Publier', ar: 'ØªØ­Ù‚Ù‚ ÙˆØ§Ù†Ø´Ø±', en: 'Validate & Publish', es: 'Validar y Publicar', pt: 'Validar e Publicar', tr: 'Onayla ve YayÄ±nla' })}
                                    </button>
                                </div>
                            )}
                        </div>
                ...
                <div className="fixed inset-0 z-[90] flex items-center justify-center p-4 bg-slate-950/20 backdrop-blur-[2px]" onClick={() => setDeleteConfirm(null)}>
                    <div className="bg-white dark:bg-dk-surface rounded-xl shadow-xl dark:shadow-dk-elevated border border-slate-200 dark:border-dk-border p-5 max-w-sm w-full" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center gap-3 mb-3">
                            ...
                            <div>
                                <h3 className="text-[14px] font-semibold text-slate-900 dark:text-dk-text">{tx(lang, { fr: "Supprimer l'ordre ?", ar: 'Ø­Ø°Ù Ø§Ù„Ø£Ù…Ø±ØŸ', en: 'Delete order?', es: 'Â¿Eliminar orden?', pt: 'Excluir ordem?', tr: 'Emir silinsin mi?' })}</h3>
                                <p className="text-[11px] text-slate-500 dark:text-dk-muted mt-0.5">{tx(lang, { fr: 'Cette action est dÃ©finitive.', ar: 'Ù‡Ø°Ø§ Ø§Ù„Ø¥Ø¬Ø±Ø§Ø¡ Ù†Ù‡Ø§Ø¦ÙŠ.', en: 'This action is irreversible.', es: 'Esta acciÃ³n es definitiva.', pt: 'Esta aÃ§Ã£o Ã© definitiva.', tr: 'Bu iÅŸlem geri alÄ±namaz.' })}</p>
                            </div>
                        </div>
            ...
            {toastMessage && (
                <div
                    className="fixed bottom-5 left-1/2 -translate-x-1/2 z-[100] flex items-center gap-2.5 px-4 py-2.5 rounded-lg shadow-lg dark:shadow-dk-lg border bg-white dark:bg-dk-surface border-slate-100 dark:border-dk-border"
            ...
            {quickActionMenu && createPortal(
                ...
                >
                    ...
                    >
                        ...
                        >
                                                            <Edit3 className="w-3.5 h-3.5" /> {tx(lang, { fr: 'Ouvrir', ar: 'ÙØªØ­', en: 'Open', es: 'Abrir', pt: 'Abrir', tr: 'AÃ§' })}
                        </button>
                        <button
                            onClick={() => {
                                const m = models.find(x => x.id === quickActionMenu.modelId);
                            ...
                            className="w-full text-left px-3 py-2 hover:bg-slate-50 dark:hover:bg-dk-elevated/60 flex items-center gap-2"
                        >
                                                            <Copy className="w-3.5 h-3.5" /> {tx(lang, { fr: 'Dupliquer', ar: 'ØªÙƒØ±Ø§Ø±', en: 'Duplicate', es: 'Duplicar', pt: 'Duplicar', tr: 'Kopyala' })}
                        </button>
                        <button
                            onClick={() => {
                                const m = models.find(x => x.id === quickActionMenu.modelId);
                            ...
                            className="w-full text-left px-3 py-2 hover:bg-slate-50 dark:hover:bg-dk-elevated/60 flex items-center gap-2"
                        >
                                                            <Layers className="w-3.5 h-3.5" /> {tx(lang, { fr: 'MÃ©thodes', ar: 'Ø§Ù„Ø·Ø±Ù‚', en: 'Methods', es: 'MÃ©todos', pt: 'MÃ©todos', tr: 'YÃ¶ntemler' })}
                        </button>
                        <div className="h-px bg-slate-100 dark:bg-dk-elevated my-1" />
                        <button
                            onClick={() => {
...
/* â”€â”€â”€â”€â”€â”€â”€ Board View (Kanban) â”€â”€â”€â”€â”€â”€â”€ */
function BoardView({
    ...
    const { lang } = useLang();
    const columns = [
        { key: 'EN_PREPARATION', label: tx(lang, { fr: 'PrÃ©paration', ar: 'ØªØ­Ø¶ÙŠØ±', en: 'Preparation', es: 'PreparaciÃ³n', pt: 'PreparaÃ§Ã£o', tr: 'HazÄ±rlÄ±k' }), color: 'border-slate-300', bg: 'bg-slate-50 dark:bg-dk-bg', icon: Clock, textColor: 'text-slate-700 dark:text-dk-text-soft', cardBorder: 'hover:border-slate-300', progressColor: 'bg-slate-400' },
        { key: 'EN_COURS', label: tx(lang, { fr: 'En Cours', ar: 'Ù‚ÙŠØ¯ Ø§Ù„ØªÙ†ÙÙŠØ°', en: 'In Progress', es: 'En Curso', pt: 'Em Andamento', tr: 'Devam Ediyor' }), color: 'border-blue-300', bg: 'bg-blue-50 dark:bg-blue-900/30', icon: PlayCircle, textColor: 'text-blue-700', cardBorder: 'hover:border-blue-300', progressColor: 'bg-blue-500' },
        { key: 'SOUS_TRAITANCE', label: tx(lang, { fr: 'ExtÃ©riorisÃ©', ar: 'Ù…Ù‚Ø§ÙˆÙ„ Ø®Ø§Ø±Ø¬ÙŠ', en: 'Subcontracted', es: 'Subcontratado', pt: 'Terceirizado', tr: 'TaÅŸeron' }), color: 'border-purple-300', bg: 'bg-purple-50 dark:bg-purple-900/30', icon: Truck, textColor: 'text-purple-700', cardBorder: 'hover:border-purple-300', progressColor: 'bg-purple-500' },
        { key: 'VALIDE', label: tx(lang, { fr: 'ValidÃ©', ar: 'ØªÙ… Ø§Ù„ØªØ­Ù‚Ù‚', en: 'Validated', es: 'Validado', pt: 'Validado', tr: 'OnaylandÄ±' }), color: 'border-emerald-300', bg: 'bg-emerald-50 dark:bg-emerald-900/30', icon: CheckCircle, textColor: 'text-emerald-700', cardBorder: 'hover:border-emerald-300', progressColor: 'bg-emerald-500' },
    ];
                            ...
                            <div className="flex-1 overflow-y-auto p-2 space-y-2">
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="w-full h-28 rounded-lg overflow-hidden bg-slate-100 dark:bg-dk-elevated mb-2.5 relative">
                                                ...
                                                {m.isPublishedToLibrary === false && (
                                                    <div className="absolute top-1.5 left-1.5">
                                                        <span className="px-1.5 py-0.5 text-[8px] font-bold uppercase bg-amber-400 text-white rounded-md shadow-sm dark:shadow-dk-sm">{tx(lang, { fr: 'Draft', ar: 'Ù…Ø³ÙˆØ¯Ø©', en: 'Draft', es: 'Borrador', pt: 'Rascunho', tr: 'Taslak' })}</span>
                                                    </div>
                                                )}
                                            </div>
                                            ...
                                            <div className="space-y-1.5">
                                                ...
                                                <p className="text-[10px] text-slate-400 dark:text-dk-muted font-medium truncate">{ref || tx(lang, { fr: 'RÃ©fÃ©rence inconnue', ar: 'Ù…Ø±Ø¬Ø¹ ØºÙŠØ± Ù…Ø¹Ø±ÙˆÙ', en: 'Unknown reference', es: 'Referencia desconocida', pt: 'ReferÃªncia desconhecida', tr: 'Bilinmeyen referans' })}</p>
                                                <div className="flex items-center justify-between">
                                                    <span className="text-[10px] font-bold text-slate-600 dark:text-dk-text-soft bg-slate-50 dark:bg-dk-bg px-1.5 py-0.5 rounded">{qte > 0 ? `${qte} pcs` : 'â€”'}</span>
                                                    {m.isPublishedToLibrary !== false && (
                                ...
                                {colModels.length === 0 && (
                                                                    <div className="text-center py-6 text-[11px] text-slate-400 dark:text-dk-muted italic">
                                                                        {tx(lang, { fr: 'Aucun ordre', ar: 'Ù„Ø§ ÙŠÙˆØ¬Ø¯ Ø£Ù…Ø±', en: 'No orders', es: 'Ninguna orden', pt: 'Nenhuma ordem', tr: 'Emir yok' })}
                                                                    </div>
                                )}
                            </div>
                        </div>
...
/* â”€â”€â”€â”€â”€â”€â”€ Stats View â”€â”€â”€â”€â”€â”€â”€ */
    ...
    return (
        <div className="p-4 md:p-6 max-w-6xl mx-auto space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <StatCard label={tx(lang, { fr: 'Total Ordres', ar: 'Ø¥Ø¬Ù…Ø§Ù„ÙŠ Ø§Ù„Ø£ÙˆØ§Ù…Ø±', en: 'Total Orders', es: 'Total Ã“rdenes', pt: 'Total Ordens', tr: 'Toplam Emirler' })} value={models.length} icon={Layers} color="bg-slate-900" />
                <StatCard label={tx(lang, { fr: 'Brouillons', ar: 'Ù…Ø³ÙˆØ¯Ø§Øª', en: 'Drafts', es: 'Borradores', pt: 'Rascunhos', tr: 'Taslaklar' })} value={stats.totalDrafts} icon={FileText} color="bg-amber-500" />
                <StatCard label={tx(lang, { fr: 'PubliÃ©s', ar: 'Ù…Ù†Ø´ÙˆØ±', en: 'Published', es: 'Publicados', pt: 'Publicados', tr: 'YayÄ±nlanan' })} value={stats.totalPublished} icon={CheckCircle2} color="bg-emerald-500" />
                <StatCard label={tx(lang, { fr: 'Total PiÃ¨ces', ar: 'Ø¥Ø¬Ù…Ø§Ù„ÙŠ Ø§Ù„Ù‚Ø·Ø¹', en: 'Total Pieces', es: 'Total Piezas', pt: 'Total PeÃ§as', tr: 'Toplam Adet' })} value={stats.totalQty} icon={PackageSearch} color="bg-indigo-500" />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="bg-white dark:bg-dk-surface rounded-xl border border-slate-200 dark:border-dk-border p-4 md:p-6">
                    <div className="flex items-center gap-2 mb-4">
                        ...
                        <h3 className="text-[14px] font-semibold text-slate-800 dark:text-dk-text">{tx(lang, { fr: 'RÃ©partition', ar: 'Ø§Ù„ØªÙˆØ²ÙŠØ¹', en: 'Distribution', es: 'DistribuciÃ³n', pt: 'DistribuiÃ§Ã£o', tr: 'DaÄŸÄ±lÄ±m' })}</h3>
                    </div>
                    <div className="flex items-center justify-center gap-6">
                        ...
                        >
                            <div className="w-20 h-20 bg-white dark:bg-dk-surface rounded-full flex items-center justify-center flex-col">
                                ...
                                <span className="text-[9px] text-slate-400 dark:text-dk-muted font-bold uppercase">{tx(lang, { fr: 'Ordres', ar: 'Ø£ÙˆØ§Ù…Ø±', en: 'Orders', es: 'Ã“rdenes', pt: 'Ordens', tr: 'Emirler' })}</span>
                            </div>
                        </div>
                    ...
                    <div className="space-y-4">
                        <div>
                            <div className="flex items-center justify-between mb-1.5">
                                <span className="text-[11px] font-semibold text-slate-500 dark:text-dk-muted">{tx(lang, { fr: 'Taux de chute moyen', ar: 'Ù…ØªÙˆØ³Ø· Ù†Ø³Ø¨Ø© Ø§Ù„ÙØ§Ù‚Ø¯', en: 'Avg waste rate', es: 'Tasa de desperdicio media', pt: 'Taxa de desperdÃ­cio mÃ©dia', tr: 'Ort. fire oranÄ±' })}</span>
                        ...
                        {/* Qty by status */}
                        <div>
                            <span className="text-[11px] font-semibold text-slate-500 dark:text-dk-muted block mb-2">{tx(lang, { fr: 'Production par statut', ar: 'Ø§Ù„Ø¥Ù†ØªØ§Ø¬ Ø­Ø³Ø¨ Ø§Ù„Ø­Ø§Ù„Ø©', en: 'Production by status', es: 'ProducciÃ³n por estado', pt: 'ProduÃ§Ã£o por status', tr: 'Duruma gÃ¶re Ã¼retim' })}</span>
                            <div className="space-y-2">
            ...
            <div className="bg-white dark:bg-dk-surface rounded-xl border border-slate-200 dark:border-dk-border p-4 md:p-6">
                <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="text-[14px] font-semibold text-slate-800 dark:text-dk-text">{tx(lang, { fr: 'DÃ©tail par Statut', ar: 'ØªÙØ§ØµÙŠÙ„ Ø­Ø³Ø¨ Ø§Ù„Ø­Ø§Ù„Ø©', en: 'Detail by Status', es: 'Detalle por Estado', pt: 'Detalhe por Status', tr: 'Duruma GÃ¶re Detay' })}</h3>
                </div>
...
/* â”€â”€â”€â”€â”€â”€â”€ Empty Dashboard (when no model selected) â”€â”€â”€â”€â”€â”€â”€ */
function EmptyDashboard({
    ...
    return (
        <div className="p-4 md:p-6 max-w-6xl mx-auto space-y-4">
                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div>
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-[10px] font-bold text-rose-400 uppercase tracking-widest">{tx(lang, { fr: 'Atelier Coupe', ar: 'ÙˆØ±Ø´Ø© Ø§Ù„Ù‚Øµ', en: 'Cutting Workshop', es: 'Taller de Corte', pt: 'Oficina de Corte', tr: 'Kesim AtÃ¶lyesi' })}</span>
                        </div>
                        <h1 className="text-2xl md:text-3xl font-bold text-white tracking-tight">{tx(lang, { fr: 'Bienvenue Ã  La Coupe', ar: 'Ù…Ø±Ø­Ø¨Ù‹Ø§ Ø¨Ùƒ ÙÙŠ Ù‚Ø³Ù… Ø§Ù„Ù‚Øµ', en: 'Welcome to Cutting Dept.', es: 'Bienvenido al Departamento de Corte', pt: 'Bem-vindo ao Corte', tr: 'Kesim DepartmanÄ±na HoÅŸ Geldiniz' })}</h1>
                        <p className="text-slate-400 dark:text-dk-muted text-[13px] mt-1.5 max-w-md leading-relaxed">
                            {tx(lang, { fr: 'GÃ©rez vos ordres de fabrication, suivez la production et optimisez vos processus de coupe textile.', ar: 'Ø¥Ø¯Ø§Ø±Ø© Ø£ÙˆØ§Ù…Ø± Ø§Ù„ØªØµÙ†ÙŠØ¹ Ø§Ù„Ø®Ø§ØµØ© Ø¨ÙƒØŒ ØªØªØ¨Ø¹ Ø§Ù„Ø¥Ù†ØªØ§Ø¬ ÙˆØªØ­Ø³ÙŠÙ† Ø¹Ù…Ù„ÙŠØ§Øª Ø§Ù„Ù‚Øµ Ø§Ù„Ù†Ø³ÙŠØ¬ÙŠ.', en: 'Manage your production orders, track production and optimize your textile cutting processes.', es: 'Gestione sus Ã³rdenes de fabricaciÃ³n, siga la producciÃ³n y optimice sus procesos de corte textil.', pt: 'Gerencie suas ordens de fabricaÃ§Ã£o, acompanhe a produÃ§Ã£o e otimize seus processos de corte tÃªxtil.', tr: 'Ãœretim emirlerinizi yÃ¶netin, Ã¼retimi takip edin ve tekstil kesim sÃ¼reÃ§lerinizi optimize edin.' })}
                        </p>
                    </div>
                    <div className="flex flex-col gap-2 shrink-0">
                        <button
                            onClick={onNew}
                            className="h-12 px-6 bg-white dark:bg-dk-surface hover:bg-slate-50 dark:hover:bg-dk-elevated/60 text-slate-900 dark:text-dk-text text-[13px] font-bold rounded-xl transition-colors inline-flex items-center gap-2 shadow-lg dark:shadow-dk-lg shadow-white/10"
                        >
                            <Plus className="w-4 h-4" strokeWidth={2.5} />
                            {tx(lang, { fr: 'Nouvel Ordre', ar: 'Ø£Ù…Ø± Ø¬Ø¯ÙŠØ¯', en: 'New Order', es: 'Nueva Orden', pt: 'Nova Ordem', tr: 'Yeni Emir' })}
                        </button>
                        <p className="text-[10px] text-slate-500 dark:text-dk-muted text-center">{tx(lang, { fr: 'Ou sÃ©lectionnez un ordre existant', ar: 'Ø£Ùˆ Ø§Ø®ØªØ± Ø£Ù…Ø±Ù‹Ø§ Ù…ÙˆØ¬ÙˆØ¯Ù‹Ø§', en: 'Or select an existing order', es: 'O seleccione una orden existente', pt: 'Ou selecione uma ordem existente', tr: 'Veya mevcut bir emir seÃ§in' })}</p>
                    </div>
                </div>
            </div>
            ...
            {/* Quick stats */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <StatCard label={tx(lang, { fr: 'Total Ordres', ar: 'Ø¥Ø¬Ù…Ø§Ù„ÙŠ Ø§Ù„Ø£ÙˆØ§Ù…Ø±', en: 'Total Orders', es: 'Total Ã“rdenes', pt: 'Total Ordens', tr: 'Toplam Emirler' })} value={models.length} icon={Layers} color="bg-slate-900" delay={0} />
                <StatCard label={tx(lang, { fr: 'PrÃ©paration', ar: 'ØªØ­Ø¶ÙŠØ±', en: 'Preparation', es: 'PreparaciÃ³n', pt: 'PreparaÃ§Ã£o', tr: 'HazÄ±rlÄ±k' })} value={prepCount} icon={Clock} color="bg-slate-500" delay={50} />
                <StatCard label={tx(lang, { fr: 'En Cours', ar: 'Ù‚ÙŠØ¯ Ø§Ù„ØªÙ†ÙÙŠØ°', en: 'In Progress', es: 'En Curso', pt: 'Em Andamento', tr: 'Devam Ediyor' })} value={activeCount} icon={PlayCircle} color="bg-blue-500" delay={100} />
                <StatCard label={tx(lang, { fr: 'ValidÃ©s', ar: 'ØªÙ… Ø§Ù„ØªØ­Ù‚Ù‚', en: 'Validated', es: 'Validados', pt: 'Validados', tr: 'Onaylanan' })} value={valCount} icon={CheckCircle} color="bg-emerald-500" delay={150} />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <StatCard label={tx(lang, { fr: 'Total PiÃ¨ces', ar: 'Ø¥Ø¬Ù…Ø§Ù„ÙŠ Ø§Ù„Ù‚Ø·Ø¹', en: 'Total Pieces', es: 'Total Piezas', pt: 'Total PeÃ§as', tr: 'Toplam Adet' })} value={totalQty} icon={PackageSearch} color="bg-indigo-500" delay={200} />
                <StatCard label={tx(lang, { fr: 'Brouillons', ar: 'Ù…Ø³ÙˆØ¯Ø§Øª', en: 'Drafts', es: 'Borradores', pt: 'Rascunhos', tr: 'Taslaklar' })} value={drafts} icon={FileText} color="bg-amber-500" delay={250} />
                <StatCard label={tx(lang, { fr: 'Statuts', ar: 'Ø§Ù„Ø­Ø§Ù„Ø§Øª', en: 'Statuses', es: 'Estados', pt: 'Status', tr: 'Durumlar' })} value={Object.keys(statusMap).length} icon={BarChart3} color="bg-purple-500" delay={300} />
            </div>
            ...
            <div className="bg-gradient-to-r from-indigo-50 to-rose-50 rounded-xl border border-indigo-100 p-4 md:p-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div className="flex items-start gap-2.5">
                        ...
                        <div>
                            <p className="text-[11px] font-semibold text-slate-700 dark:text-dk-text-soft">{tx(lang, { fr: 'CrÃ©er un ordre', ar: 'Ø¥Ù†Ø´Ø§Ø¡ Ø£Ù…Ø±', en: 'Create an order', es: 'Crear una orden', pt: 'Criar uma ordem', tr: 'Emir oluÅŸtur' })}</p>
                            <p className="text-[10px] text-slate-500 dark:text-dk-muted">{tx(lang, { fr: 'DÃ©finissez les paramÃ¨tres de matelas', ar: 'Ø­Ø¯Ø¯ Ø¥Ø¹Ø¯Ø§Ø¯Ø§Øª Ø§Ù„Ù…ÙØ±Ø´Ø©', en: 'Set layering parameters', es: 'Defina los parÃ¡metros de capas', pt: 'Defina os parÃ¢metros de esteiramento', tr: 'Katman parametrelerini ayarlayÄ±n' })}</p>
                        </div>
                    </div>
                    <div className="flex items-start gap-2.5">
                        <div className="w-6 h-6 rounded bg-indigo-100 flex items-center justify-center shrink-0 mt-0.5">
                        ...
                        <div>
                            <p className="text-[11px] font-semibold text-slate-700 dark:text-dk-text-soft">{tx(lang, { fr: 'RÃ©partir tailles/couleurs', ar: 'ØªÙˆØ²ÙŠØ¹ Ø§Ù„Ù…Ù‚Ø§Ø³Ø§Øª/Ø§Ù„Ø£Ù„ÙˆØ§Ù†', en: 'Distribute sizes/colors', es: 'Distribuir tallas/colores', pt: 'Distribuir tamanhos/cores', tr: 'Bedenleri/renkleri daÄŸÄ±t' })}</p>
                            <p className="text-[10px] text-slate-500 dark:text-dk-muted">{tx(lang, { fr: 'Remplissez la matrice de production', ar: 'Ø§Ù…Ù„Ø£ Ù…ØµÙÙˆÙØ© Ø§Ù„Ø¥Ù†ØªØ§Ø¬', en: 'Fill the production matrix', es: 'Complete la matriz de producciÃ³n', pt: 'Preencha a matriz de produÃ§Ã£o', tr: 'Ãœretim matrisini doldurun' })}</p>
                        </div>
                    </div>
                    <div className="flex items-start gap-2.5">
                        <div className="w-6 h-6 rounded bg-indigo-100 flex items-center justify-center shrink-0 mt-0.5">
                        ...
                        <div>
                            <p className="text-[11px] font-semibold text-slate-700 dark:text-dk-text-soft">{tx(lang, { fr: 'Publier & Valider', ar: 'Ù†Ø´Ø± ÙˆØªØ­Ù‚Ù‚', en: 'Publish & Validate', es: 'Publicar y Validar', pt: 'Publicar e Validar', tr: 'YayÄ±nla ve Onayla' })}</p>
                            <p className="text-[10px] text-slate-500 dark:text-dk-muted">{tx(lang, { fr: 'Envoyez vers le Planning', ar: 'Ø£Ø±Ø³Ù„ Ø¥Ù„Ù‰ Ø§Ù„ØªØ®Ø·ÙŠØ·', en: 'Send to Planning', es: 'Enviar a PlanificaciÃ³n', pt: 'Enviar para Planejamento', tr: 'Planlamaya gÃ¶nder' })}</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div className="bg-white dark:bg-dk-surface rounded-xl border border-slate-200 dark:border-dk-border overflow-hidden">
                    <div className="px-4 md:px-6 py-4 border-b border-slate-100 dark:border-dk-border flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            ...
                            <h3 className="text-[14px] font-semibold text-slate-800 dark:text-dk-text">{tx(lang, { fr: 'Ordres RÃ©cents', ar: 'Ø§Ù„Ø£ÙˆØ§Ù…Ø± Ø§Ù„Ø£Ø®ÙŠØ±Ø©', en: 'Recent Orders', es: 'Ã“rdenes Recientes', pt: 'Ordens Recentes', tr: 'Son Emirler' })}</h3>
                        </div>
                        <span className="text-[11px] text-slate-400 dark:text-dk-muted font-medium">{recent.length} {tx(lang, { fr: 'derniers', ar: 'Ø§Ù„Ø£Ø®ÙŠØ±Ø©', en: 'latest', es: 'Ãºltimos', pt: 'Ãºltimos', tr: 'son' })}</span>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x divide-slate-100 dark:divide-dk-border">
                        {recent.map(m => {
                            const st = m.ordreCoupe?.status || 'EN_PREPARATION';
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex items-center gap-1.5 mt-0.5">
                                                ...
                                                {qte > 0 && (
                                                    <>
                                                        <span className="text-slate-300 dark:text-dk-muted text-[7px]">Â·</span>
            ...
            {models.length === 0 && (
                <div className="bg-white dark:bg-dk-surface rounded-xl border-2 border-dashed border-slate-200 dark:border-dk-border p-10 text-center">
                    ...
                    <h3 className="text-[15px] font-bold text-slate-700 dark:text-dk-text-soft">{tx(lang, { fr: 'Aucun ordre de coupe', ar: 'Ù„Ø§ ÙŠÙˆØ¬Ø¯ Ø£Ù…Ø± Ù‚Øµ', en: 'No cutting order', es: 'Ninguna orden de corte', pt: 'Nenhuma ordem de corte', tr: 'Kesim emri yok' })}</h3>
                    <p className="text-[12px] text-slate-400 dark:text-dk-muted mt-1.5 max-w-sm mx-auto leading-relaxed">
                        {tx(lang, { fr: 'Commencez par crÃ©er votre premier ordre de fabrication pour suivre vos productions textile.', ar: 'Ø§Ø¨Ø¯Ø£ Ø¨Ø¥Ù†Ø´Ø§Ø¡ Ø£ÙˆÙ„ Ø£Ù…Ø± ØªØµÙ†ÙŠØ¹ Ù„Ùƒ Ù„Ù…ØªØ§Ø¨Ø¹Ø© Ø¥Ù†ØªØ§Ø¬Ø§ØªÙƒ Ø§Ù„Ù†Ø³ÙŠØ¬ÙŠØ©.', en: 'Start by creating your first production order to track your textile productions.', es: 'Comience creando su primera orden de fabricaciÃ³n para seguir sus producciones textiles.', pt: 'Comece criando sua primeira ordem de fabricaÃ§Ã£o para acompanhar suas produÃ§Ãµes tÃªxteis.', tr: 'Tekstil Ã¼retimlerinizi takip etmek iÃ§in ilk Ã¼retim emrinizi oluÅŸturarak baÅŸlayÄ±n.' })}
                    </p>
                    <button
                        onClick={onNew}
                        className="mt-5 h-11 px-6 bg-slate-900 hover:bg-slate-800 text-white text-[12px] font-bold rounded-xl transition-colors inline-flex items-center gap-2 shadow-lg dark:shadow-dk-lg shadow-slate-900/20"
                    >
                        <Plus className="w-4 h-4" strokeWidth={2.5} />
                        {tx(lang, { fr: 'CrÃ©er un ordre', ar: 'Ø¥Ù†Ø´Ø§Ø¡ Ø£Ù…Ø±', en: 'Create order', es: 'Crear orden', pt: 'Criar ordem', tr: 'Emir oluÅŸtur' })}
                    </button>
                </div>
            )}
        </div>
...
/* â”€â”€â”€â”€â”€â”€â”€ Shared Input Component â”€â”€â”€â”€â”€â”€â”€ */
function InputField({
