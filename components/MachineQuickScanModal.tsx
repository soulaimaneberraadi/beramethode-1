        ...
        <div className="p-5 flex flex-col gap-4 overflow-y-auto max-h-[80vh]">
          ...
          {!identified && (
            <>
              <div className="flex rounded-xl border border-slate-200 overflow-hidden text-[11px] font-black uppercase tracking-widest">
                <button onClick={() => setTab('mat')} className={`flex-1 py-2 transition-colors ${tab === 'mat' ? 'bg-indigo-600 text-white' : 'bg-slate-50 text-slate-500 hover:bg-slate-100'}`}>{tx(lang, { fr: 'Matricule', ar: 'الرقم التسلسلي', en: 'Serial number', es: 'Matrícula', pt: 'Matrícula', tr: 'Seri numarası' })}</button>
          ...
          {done && (
            <div className="flex flex-col items-center justify-center gap-3 py-6">
              ...
              <div className="text-xs text-slate-400">{tx(lang, { fr: 'L\'historique a été mis à jour.', ar: 'تم تحديث السجل.', en: 'The history has been updated.', es: 'El historial ha sido actualizado.', pt: 'O histórico foi atualizado.', tr: 'Geçmiş güncellendi.' })}</div>
            </div>
          )}
        </div>
      </div>
