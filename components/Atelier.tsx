                    ...
                    <div className="space-y-6 animate-in fade-in zoom-in-95 duration-300">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                            <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm flex items-center gap-4">
                                ...
                                <div>
                                    <p className="text-sm font-bold text-slate-500">{tx(lang, { fr: 'OF en cours', ar: 'OF قيد التنفيذ', en: 'OF in progress', es: 'OF en curso', pt: 'OF em andamento', tr: 'Devam eden OF' })}</p>
                            ...
                            <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm flex items-center gap-4">
                                ...
                                <div>
                                    <p className="text-sm font-bold text-slate-500">{tx(lang, { fr: 'Total Pièces Prévues', ar: 'إجمالي القطع المخططة', en: 'Total Planned Pieces', es: 'Total Piezas Previstas', pt: 'Total de Peças Previstas', tr: 'Toplam Planlanan Parça' })}</p>
                            ...
                            <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm flex items-center gap-4">
                                ...
                                <div>
                                    <p className="text-sm font-bold text-slate-500">{tx(lang, { fr: 'OF Terminés (Total)', ar: 'OF منتهية (الإجمالي)', en: 'Completed OF (Total)', es: 'OF Terminadas (Total)', pt: 'OF Concluídas (Total)', tr: 'Tamamlanan OF (Toplam)' })}</p>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            {todayEvents.length === 0 ? (
                                <div className="col-span-full py-12 text-center bg-white rounded-3xl border border-slate-200 border-dashed">
                                    ...
                                    <p className="text-slate-500 font-bold">{tx(lang, { fr: 'Aucun lancement prévu pour cette journée.', ar: 'لا يوجد إطلاق مبرمج لهذا اليوم.', en: 'No launch scheduled for this day.', es: 'Ningún lanzamiento previsto para este día.', pt: 'Nenhum lançamento previsto para este dia.', tr: 'Bu gün için planlanmış bir başlatma yok.' })}</p>
                                </div>
                            ) : (
                                todayEvents.map(evt => {
                                        ...
                                        <div key={evt.id} className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm flex flex-col hover:shadow-md transition-shadow">
                                            ...
                                            <div className="mt-auto">
                                                <div className="flex justify-between text-xs font-bold mb-1">
                                                    <span className="text-slate-500">{tx(lang, { fr: 'Avancement', ar: 'التقدّم', en: 'Progress', es: 'Progreso', pt: 'Progresso', tr: 'İlerleme' })} ({totalProduced}/{evt.qteTotal})</span>
                ...
                {tab === 'demandes' && (
                    <div className="animate-in fade-in zoom-in-95 duration-300">
                        <div className="bg-white p-8 rounded-3xl border shadow-sm max-w-2xl mx-auto">
                            ...
                            <p className="text-slate-500 font-bold text-sm mb-6">{tx(lang, { fr: 'Créez un ticket pour demander de la matière supplémentaire au magasin de manière tracée.', ar: 'أنشئ تذكرة لطلب مواد إضافية من المخزن بشكل متتبَّع.', en: 'Create a ticket to request additional material from the warehouse in a tracked way.', es: 'Cree un ticket para solicitar material adicional al almacén de forma trazable.', pt: 'Crie um chamado para solicitar material adicional ao armazém de forma rastreável.', tr: 'Depodan ek malzeme talep etmek için izlenebilir bir talep oluşturun.' })}</p>
                            ...
                            {demandeSent ? (
                                <div className="py-12 text-center">
                                    ...
                                    <p className="text-slate-500 font-bold mt-2">{tx(lang, { fr: 'Le magasinier a reçu votre demande et peut la valider depuis son module.', ar: 'استلم أمين المخزن طلبك ويمكنه التحقق منه من وحدته.', en: 'The storekeeper has received your request and can validate it from their module.', es: 'El almacenero recibió su solicitud y puede validarla desde su módulo.', pt: 'O almoxarife recebeu seu pedido e pode validá-lo em seu módulo.', tr: 'Depo sorumlusu talebinizi aldı ve kendi modülünden onaylayabilir.' })}</p>
                                </div>
                            ) : (
                                <div className="space-y-5">
                                    <div>
                                        <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5">{tx(lang, { fr: "Pour l'OF / Modèle", ar: 'لأجل OF / النموذج', en: 'For the OF / Model', es: 'Para el OF / Modelo', pt: 'Para a OF / Modelo', tr: 'OF / Model için' })} *</label>
                                        <select
                                            value={demandeOF}
                                    ...
                                    <div>
                                        <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5">{tx(lang, { fr: 'Article Demandé (Référence Magasin)', ar: 'المادة المطلوبة (مرجع المخزن)', en: 'Requested Article (Warehouse Reference)', es: 'Artículo Solicitado (Referencia de Almacén)', pt: 'Artigo Solicitado (Referência de Armazém)', tr: 'Talep Edilen Ürün (Depo Referansı)' })} *</label>
                                        <input
                                            type="text"
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5">{tx(lang, { fr: 'Quantité', ar: 'الكمية', en: 'Quantity', es: 'Cantidad', pt: 'Quantidade', tr: 'Miktar' })} *</label>
                                            <input
                                                type="number"
                                                min="1"
                                                value={demandeQty || ''}
                                        ...
                                        <div>
                                            <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5">{tx(lang, { fr: 'Demandeur', ar: 'الطالب', en: 'Requester', es: 'Solicitante', pt: 'Solicitante', tr: 'Talep Eden' })}</label>
                                            <input
                                                type="text"
                                    ...
                                    <div>
                                        <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5">{tx(lang, { fr: 'Motif / Note', ar: 'السبب / ملاحظة', en: 'Reason / Note', es: 'Motivo / Nota', pt: 'Motivo / Nota', tr: 'Sebep / Not' })}</label>
                                        <textarea
                                            value={demandeMotif}
                        ...
                        <div className="bg-white p-8 rounded-3xl border shadow-sm max-w-3xl mx-auto border-t-4 border-t-rose-500">
                            ...
                            {clotureDone ? (
                                <div className="py-12 text-center">
                                    ...
                                    <p className="text-slate-500 font-bold mt-2">{tx(lang, { fr: 'Le statut a été mis à jour, les retours matière ont été enregistrés dans le Magasin.', ar: 'تم تحديث الحالة، وتم تسجيل مرتجعات المواد في المخزن.', en: 'The status has been updated, the material returns have been recorded in the Warehouse.', es: 'El estado ha sido actualizado, las devoluciones de material han sido registradas en el Almacén.', pt: 'O status foi atualizado, as devoluções de material foram registradas no Armazém.', tr: 'Durum güncellendi, malzeme iadeleri Depoya kaydedildi.' })}</p>
                                </div>
                            ) : (
                                <div className="space-y-6">
                                    <div>
                                        <label className="block text-xs font-black text-slate-700 uppercase tracking-widest mb-2">{tx(lang, { fr: "1. Sélectionner l'OF à clôturer", ar: '1. حدّد OF لإغلاقه', en: '1. Select the OF to close', es: '1. Seleccionar el OF a cerrar', pt: '1. Selecionar a OF a encerrar', tr: '1. Kapatılacak OF\'yi seçin' })}</label>
                                        <select
                                            value={clotureOF}
                                    ...
                                    <div className="grid grid-cols-2 gap-6 p-6 bg-slate-50 rounded-2xl border border-slate-200">
                                        <div>
                                            <label className="block text-xs font-black text-emerald-700 uppercase tracking-widest mb-2">{tx(lang, { fr: 'Pièces Bonnes (1er Choix)', ar: 'القطع الجيدة (الاختيار الأول)', en: 'Good Pieces (1st Choice)', es: 'Piezas Buenas (1ª Calidad)', pt: 'Peças Boas (1ª Escolha)', tr: 'İyi Parçalar (1. Seçim)' })}</label>
                                            <input
                                                type="number"
                                                value={piecesBonnes || ''}
                                        ...
                                        <div>
                                            <label className="block text-xs font-black text-amber-700 uppercase tracking-widest mb-2">{tx(lang, { fr: 'Rebuts / 2ème Choix', ar: 'النفايات / الاختيار الثاني', en: 'Rejects / 2nd Choice', es: 'Rechazos / 2ª Calidad', pt: 'Rejeitos / 2ª Escolha', tr: 'Hurda / 2. Seçim' })}</label>
                                            <input
                                                type="number"
                                                value={piecesRebut || ''}
                                    ...
                                    <div className="p-6 bg-indigo-50 rounded-2xl border border-indigo-100">
                                        ...
                                        <div className="flex gap-4 items-center">
                                            ...
                                            />
                                            <span className="font-bold text-indigo-600">{tx(lang, { fr: 'Heures de travail (par O. direct)', ar: 'ساعات العمل (يدوي مباشر)', en: 'Work hours (direct labor)', es: 'Horas de trabajo (mano de obra directa)', pt: 'Horas de trabalho (mão de obra direta)', tr: 'Çalışma saatleri (doğrudan işçilik)' })}</span>
                                        </div>
                                    </div>
