...
export function HRWorkerProfilePanel({ workerId, onClose, onEdit, settings }: Props) {
  ...
  if (loading && !dossier) {
    return (
      ...
      >
        <div style={{ color: '#64748B', fontSize: 14 }}>{_({fr:'Chargement du dossier…',ar:'جاري تحميل الملف…',en:'Loading file…',es:'Cargando expediente…',pt:'Carregando dossiê…',tr:'Dosya yükleniyor…'})}</div>
      </motion.div>
    );
  }
          ...
          <>
            ...
            <div style={{ ...card, marginBottom: 14 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, maxWidth: 400 }}>
                <div>
                  <div style={labelStyle}>{_({fr:'Nouveau PIN',ar:'رقم التعريف الجديد',en:'New PIN',es:'Nuevo PIN',pt:'Novo PIN',tr:'Yeni PIN'})}</div>
                  <input type="password" value={pin1} onChange={e => setPin1(e.target.value)} style={{ ...inputStyle, width: '100%' }} placeholder={_({fr:'4–8 chiffres',ar:'4-8 أرقام',en:'4–8 digits',es:'4–8 dígitos',pt:'4–8 dígitos',tr:'4-8 hane'})} />
                </div>
                <div>
                  <div style={labelStyle}>{_({fr:'Confirmer',ar:'تأكيد',en:'Confirm',es:'Confirmar',pt:'Confirmar',tr:'Onayla'})}</div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr>
                    ...
                    ].map(h => <th key={h.k} style={th}>{h.v}</th>)}
                  </tr>
                </thead>
                <tbody>
                  {dossier.production.length === 0 && (
                    <tr><td colSpan={7} style={{ ...td, color: '#94A3B8', textAlign: 'center' }}>{_({fr:'Aucune ligne de production',ar:'لا يوجد خط إنتاج',en:'No production lines',es:'Sin líneas de producción',pt:'Nenhuma linha de produção',tr:'Üretim hattı yok'})}</td></tr>
                  )}
