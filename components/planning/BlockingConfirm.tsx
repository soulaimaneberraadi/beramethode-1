    ...
    return (
        <div className="fixed inset-0 z-[400] flex items-center justify-center bg-slate-900/50 p-4" role="dialog" aria-modal="true">
            <div className="w-full max-w-sm rounded-2xl border border-slate-200 dark:border-dk-border bg-white dark:bg-dk-surface p-5 shadow-2xl">
                ...
                <p className="text-sm text-slate-600 dark:text-dk-text-soft mb-5 whitespace-pre-wrap break-words">{message}</p>
                <div className="flex gap-2">
                    <button type="button" onClick={onCancel} className="flex-1 rounded-xl border border-slate-200 dark:border-dk-border py-2.5 text-sm font-bold text-slate-700 dark:text-dk-text-soft hover:bg-slate-50 dark:hover:bg-dk-elevated/60">
                        {cancelLabel}
                    </button>
