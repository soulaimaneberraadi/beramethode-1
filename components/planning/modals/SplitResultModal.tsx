    ...
    return (
        ...
        >
            <div className="space-y-5">
                {/* Success banner */}
                <div className="bg-emerald-50 dark:bg-emerald-900/30 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-xl p-4 flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-emerald-100 dark:bg-emerald-900/40 flex items-center justify-center">
                        <CheckCircle2 className="w-5 h-5 text-emerald-600 dark:text-emerald-400" />
                    </div>
                    <div>
                        <div className="text-[13px] font-bold text-emerald-800 dark:text-emerald-400">{tx(lang,{fr:'Fractionnement réussi',ar:'تم التقسيم بنجاح',en:'Split successful',es:'División exitosa',pt:'Divisão bem-sucedida',tr:'Bölme başarılı'})}</div>
                        <div className="text-[11px] text-emerald-600 dark:text-emerald-400 dark:text-emerald-300">
                ...
                <div className="flex items-center justify-center gap-3 py-2">
                    ...
                    <div className="flex flex-col items-center gap-1">
                        ...
                        <div className="text-[9px] font-bold text-indigo-600 dark:text-indigo-400 dark:text-dk-accent-text dark:text-indigo-400">{tx(lang,{fr:'SPLIT',ar:'تقسيم',en:'SPLIT',es:'DIVIDIR',pt:'DIVIDIR',tr:'BÖL'})}</div>
                    </div>

                    {/* After */}
                    <div className="flex-1 bg-indigo-50 dark:bg-indigo-900/30 dark:bg-dk-accent/20 dark:bg-indigo-900/20 rounded-xl p-3 text-center border border-indigo-200 dark:border-indigo-800">
                        ...
                        <div className="text-[10px] text-indigo-600 dark:text-indigo-400 dark:text-dk-accent-text dark:text-indigo-400">{tx(lang,{fr:'pcs total',ar:'إجمالي القطع',en:'pcs total',es:'pcs total',pt:'pcs total',tr:'toplam adet'})}</div>
                    </div>
                </div>

                {/* Original order (updated) */}
                <div className="bg-slate-50 dark:bg-dk-bg border border-slate-200 dark:border-dk-border rounded-xl p-4 space-y-3">
                    <div className="flex items-center gap-2">
                        ...
                        <div className="text-[12px] font-bold text-slate-900 dark:text-dk-text">{tx(lang,{fr:'Ordre original (mis à jour)',ar:'الطلب الأصلي (مُحدّث)',en:'Original order (updated)',es:'Pedido original (actualizado)',pt:'Pedido original (atualizado)',tr:'Orijinal sipariş (güncellendi)'})}</div>
                        <div className="ml-auto px-2 py-0.5 bg-slate-200 dark:bg-dk-border rounded-full text-[10px] font-bold text-slate-600 dark:text-dk-muted">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div className="flex items-center gap-2">
                            <Hash className="w-3.5 h-3.5 text-slate-400 dark:text-dk-muted" />
                            <div>
                                <div className="text-[9px] font-bold text-slate-500 dark:text-dk-muted uppercase">{tx(lang,{fr:'Quantité',ar:'الكمية',en:'Quantity',es:'Cantidad',pt:'Quantidade',tr:'Miktar'})}</div>
                                <div className="text-[14px] font-black text-slate-900 dark:text-dk-text tabular-nums">{originalQty} pcs</div>
                            </div>
                        </div>
                        <div className="flex items-center gap-2">
                            <Calendar className="w-3.5 h-3.5 text-slate-400 dark:text-dk-muted" />
                            <div>
                                <div className="text-[9px] font-bold text-slate-500 dark:text-dk-muted uppercase">{tx(lang,{fr:'Fin estimée',ar:'النهاية المتوقعة',en:'Estimated end',es:'Fin estimado',pt:'Fim estimado',tr:'Tahmini bitiş'})}</div>
                                <div className="text-[12px] font-semibold text-slate-900 dark:text-dk-text">{endYmd || '—'}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    {ddsYmd && (
                        <div className="flex items-center gap-2 text-[11px]">
                            <span className="text-slate-500 dark:text-dk-muted">DDS:</span>
                            <span className="font-semibold text-slate-900 dark:text-dk-text">{ddsYmd}</span>
                ...
                {newEvents.map((newEv, idx) => {
                    ...
                    return (
                        <div key={newEv.id} className="bg-indigo-50 dark:bg-indigo-900/30 dark:bg-dk-accent/20 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800 rounded-xl p-4 space-y-3">
                            <div className="flex items-center gap-2">
                ...
                <div className="space-y-2">
                    <div className="text-[10px] font-bold text-slate-500 dark:text-dk-muted uppercase tracking-wider">{tx(lang,{fr:'Répartition',ar:'التوزيع',en:'Distribution',es:'Distribución',pt:'Distribuição',tr:'Dağılım'})}</div>
                    <div className="flex h-3 rounded-full overflow-hidden bg-slate-200 dark:bg-dk-border">
                        <div
                            className="bg-slate-600 dark:bg-dk-accent transition-all"
                    ...
                    <div className="flex gap-4 text-[11px]">
                        <div className="flex items-center gap-1.5">
                            <div className="w-3 h-3 rounded bg-slate-600 dark:bg-dk-accent" />
                            <span className="text-slate-600 dark:text-dk-text-soft">{tx(lang,{fr:'Original:',ar:'الأصلي:',en:'Original:',es:'Original:',pt:'Original:',tr:'Orijinal:'})} <span className="font-bold">{originalQty} pcs</span></span>
                        </div>
                        {newEvents.map((newEv, idx) => (
                            <div key={newEv.id} className="flex items-center gap-1.5">
                                <div className="w-3 h-3 rounded bg-indigo-500" />
                                <span className="text-indigo-600 dark:text-indigo-400 dark:text-dk-accent-text dark:text-indigo-400">{tx(lang,{fr:'Nouveau',ar:'جديد',en:'New',es:'Nuevo',pt:'Novo',tr:'Yeni'})} {idx + 1}: <span className="font-bold">{evQty(newEv)} pcs</span></span>
                            </div>
                        ))}
                    </div>
                </div>
