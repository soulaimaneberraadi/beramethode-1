        ...
        >
          ...
          >
              ...
              <div className="flex flex-col md:flex-row items-center md:items-start justify-between gap-6">
                <div className="flex flex-col md:flex-row items-center gap-6">
                  ...
                  <div className="text-center md:text-left">
                    ...
                    <div className="flex flex-wrap justify-center md:justify-start items-center gap-3 mt-2">
                       ...
                       {employee.chaineId && (
                         <span className="text-sm font-bold text-amber-600 bg-amber-50 px-3 py-1 rounded-full">{tx(lang, {fr:'Chaîne:', ar:'الخط:', en:'Line:', es:'Línea:', pt:'Linha:', tr:'Hat:'})} {employee.chaineId}</span>
                       )}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap justify-center gap-4">
                  <div className="bg-white border border-slate-100 shadow-sm rounded-2xl p-4 flex flex-col items-center min-w-[120px]">
                    <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-1">{tx(lang, {fr:'Score Performance', ar:'درجة الأداء', en:'Performance Score', es:'Puntuación de Rendimiento', pt:'Pontuação de Desempenho', tr:'Performans Skoru'})}</span>
                  ...
                  <div className="bg-white border border-slate-100 shadow-sm rounded-2xl p-4 flex flex-col items-center min-w-[120px]">
                    <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-1">{tx(lang, {fr:'Activités du Mois', ar:'أنشطة الشهر', en:'Monthly Activities', es:'Actividades del Mes', pt:'Atividades do Mês', tr:'Aylık Aktiviteler'})}</span>
                    <span className="text-2xl font-black text-indigo-600 leading-none">{data.monthDone}</span>
                    <span className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-tighter">{tx(lang, {fr:'Tasks Termitées', ar:'المهام المنجزة', en:'Tasks Completed', es:'Tareas Completadas', pt:'Tarefas Concluídas', tr:'Tamamlanan Görevler'})}</span>
                  </div>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-hidden flex flex-col lg:flex-row gap-0">
              ...
              <div className="flex-1 bg-white p-8 overflow-hidden flex flex-col">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <p className="text-xs font-bold text-slate-400">{tx(lang, {fr:'Toutes les tâches terminées', ar:'جميع المهام المنتهية', en:'All completed tasks', es:'Todas las tareas terminadas', pt:'Todas as tarefas concluídas', tr:'Tamamlanan tüm görevler'})}</p>
                </div>
                    ...
                    <div key={task.id} className="relative pl-8 pb-4 group last:pb-0">
                      ...
                      <div className="p-5 rounded-3xl border border-slate-100 bg-white hover:border-emerald-200 hover:shadow-lg transition-all">
                        <div className="flex justify-between items-start mb-1">
                          ...
                          <span className="text-[10px] font-black text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded">{tx(lang, {fr:'TERMINÉ', ar:'مُنْجَز', en:'COMPLETED', es:'TERMINADO', pt:'CONCLUÍDO', tr:'TAMAMLANDI'})}</span>
                        </div>
                        <p className="text-xs text-slate-500 mb-3">{task.description || tx(lang, {fr:'Action réalisée conformément aux instructions.', ar:'تم تنفيذ الإجراء وفقاً للتعليمات.', en:'Action completed as per instructions.', es:'Acción realizada según instrucciones.', pt:'Ação realizada conforme as instruções.', tr:'İşlem talimatlara uygun şekilde gerçekleştirildi.'})}</p>
                        <div className="flex items-center gap-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                          <span className="flex items-center gap-1"><Clock3 className="w-3 h-3 text-slate-300" /> {formatDate(task.completedAt, lang)}</span>
                          <span className="flex items-center gap-1"><User className="w-3 h-3 text-slate-300" /> {tx(lang, {fr:'Par:', ar:'بواسطة:', en:'By:', es:'Por:', pt:'Por:', tr:'Tarafından:'})} {task.assignedBy}</span>
                        </div>
                      </div>
                    </div>
                  ))}
            ...
            <div className="px-8 py-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
              <p className="text-[10px] font-bold text-slate-400 tracking-widest uppercase">{tx(lang, {fr:'Expert RH Module • BERAMETHODE', ar:'خبير الموارد البشرية • BERAMETHODE', en:'HR Expert Module • BERAMETHODE', es:'Módulo Experto RH • BERAMETHODE', pt:'Módulo Especialista RH • BERAMETHODE', tr:'İK Uzman Modülü • BERAMETHODE'})}</p>
