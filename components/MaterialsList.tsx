...
const MaterialsList: React.FC<MaterialsListProps> = ({
    ...
    const ScopeChip = ({ active, onClick, children, hex }: { active: boolean; onClick: () => void; children: React.ReactNode; hex?: string | null }) => (
        <button type="button" onClick={onClick}
            className={`inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-full border text-[10px] font-medium transition-colors ${active ? 'border-slate-300 bg-slate-100 text-slate-700' : 'border-slate-200 bg-white text-slate-500 hover:bg-slate-50'}`}>
            ...
            <div className={`border-b ${darkMode ? 'border-gray-700' : 'border-slate-100'} animate-in fade-in duration-150`}>
                ...
                <div className="p-3">
                    ...
                    <div className="flex items-start justify-between gap-2 mb-2">
                        <div className="flex-1 relative">
                            ...
                            {focusedRow === item.id && (filteredMagasin.length > 0 || (item.name && filteredMagasin.length === 0)) && (
                                <div className="absolute z-[100] left-0 right-0 top-[38px] bg-white border border-slate-200 rounded-lg shadow-lg max-h-48 overflow-y-auto animate-in fade-in">
                                    {filteredMagasin.length > 0 ? filteredMagasin.map(m => (
                                        <div key={m.id}
                                            ...
                                            <div className="flex justify-between items-start">
                                                ...
                                                <span className="font-bold text-[10px] text-[#2149C1] bg-slate-100 px-1.5 py-0.5 rounded">{(m.prixUnitaire || 0).toFixed(2)} {currency}</span>
                                            </div>
                        ...
                        <div className="flex-1">
                            ...
                            {isBobine ? (
                                <div className={`${inputCls} text-center font-mono text-[13px] flex items-center justify-between`}>
                                    ...
                                    <button onClick={() => setExpandedBobine(bobineOpen ? null : item.id)} className="text-slate-400 hover:text-[#2149C1]">
                    ...
                    {canAssign && expandedScope === item.id && (
                        <div className="mt-2 p-2 bg-slate-100/40 border border-slate-200 rounded-lg animate-in fade-in duration-150">
    ...
    <React.Fragment>
        <tr className={`group ${darkMode ? 'hover:bg-gray-700/50' : 'hover:bg-slate-50'} transition-colors`}>
            ...
            <td className="px-3 py-1.5 align-top relative">
                ...
                {focusedRow === item.id && (filteredMagasin.length > 0 || (item.name && filteredMagasin.length === 0)) && (
                    <div className="absolute z-[100] left-0 right-0 top-[34px] bg-white border border-slate-200 rounded-lg shadow-lg max-h-48 overflow-y-auto animate-in fade-in">
                        {filteredMagasin.length > 0 ? filteredMagasin.slice(0, 6).map(m => (
                            <div key={m.id}
                                ...
                                <div className="text-right shrink-0 ml-2">
                                    <span className="font-bold text-[10px] text-[#2149C1] bg-slate-100 px-1.5 py-0.5 rounded">{(m.prixUnitaire || 0).toFixed(2)}</span>
        ...
        {canAssign && expandedScope === item.id && (
            <tr>
                <td colSpan={5} className="px-3 py-2 bg-slate-100/40 border-b border-slate-200">
