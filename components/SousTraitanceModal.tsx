...
import { Factory, Scissors, Package, Coins, Check } from 'lucide-react';
...
const SousTraitanceModal: React.FC<SousTraitanceModalProps> = ({ currency, value, onApply, onClose }) => {
    ...
    return (
        /* Fenêtre de SAISIE (prix à la pièce) : `closeOnBackdrop={false}` — un
           clic à côté ne doit pas effacer un prix qu'on vient de taper. Pas de
           plein écran : deux choix et un champ, agrandir n'apporterait rien. */
        <SheetModal
            onClose={onClose}
            zClass="z-[110]"
            size="lg"
            closeOnBackdrop={false}
            icon={<Factory className="w-4 h-4 text-slate-400 dark:text-dk-muted" strokeWidth={1.75} />}
            title={tx(lang, {fr:"Sous-traitance",ar:"المقاولة من الباطن",en:"Subcontracting",es:"Subcontratación",pt:"Subcontratação",tr:"Taşeronluk"})}
            subtitle="Modèle confié à un façonnier"
            footer={
                <>
                    <button
                        onClick={onClose}
        ...
        >
            {/* `dir="ltr"` conservé sur le corps : les montants et le champ de
                prix se lisent de gauche à droite même en arabe. */}
