                ...
                <div className="px-4 sm:px-5 h-12 border-b border-slate-100 flex items-center justify-between shrink-0">
                    <div className="flex items-center gap-2 min-w-0">
                        ...
                        <div className="min-w-0">
                            ...
                            {material.colorName && (
                                <p className="text-[11px] text-slate-400 truncate">{tx(lang, {fr:'Couleur :', ar:'اللون :', en:'Color:', es:'Color:', pt:'Cor:', tr:'Renk:'})} {material.colorName}</p>
                            )}
                        </div>
                    </div>
                ...
                <div className="p-3 sm:p-5 space-y-3 sm:space-y-4 overflow-y-auto">
                    <div className={`flex items-center gap-2.5 p-3 rounded-md border ${status.bg} ${status.border}`}>
                        ...
                        <div className="min-w-0">
                            ...
                            {isDelivered && (
                                <p className="text-[11px] text-slate-500 mt-0.5">{tx(lang, {fr:'Stock suffisant :', ar:'مخزون كاف:', en:'Sufficient stock:', es:'Stock suficiente:', pt:'Stock suficiente:', tr:'Yeterli stok:'})} {fmt(stockActuel)} {material.unit}</p>
                            )}
                            {!isDelivered && !isPartial && (
                                <p className="text-[11px] text-slate-500 mt-0.5">{fmt(manque)} {material.unit} {tx(lang, {fr:'manquants', ar:'مفقودة', en:'missing', es:'faltantes', pt:'faltando', tr:'eksik'})}</p>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2.5 sm:gap-3">
                        <div className="bg-slate-50/60 rounded-md p-3 border border-slate-200">
                            <span className="text-[10px] font-medium text-slate-500 uppercase tracking-wider block mb-1">{tx(lang, {fr:'Prix Unitaire HT', ar:'السعر الوحدوي HT', en:'Unit Price HT', es:'Precio Unitario HT', pt:'Preço Unitário HT', tr:'Birim Fiyat HT'})}</span>
                        ...
                        <div className="bg-slate-50/60 rounded-md p-3 border border-slate-200">
                            <span className="text-[10px] font-medium text-slate-500 uppercase tracking-wider block mb-1">{tx(lang, {fr:'Quantité à acheter', ar:'الكمية المطلوب شراؤها', en:'Quantity to buy', es:'Cantidad a comprar', pt:'Quantidade a comprar', tr:'Satın alınacak miktar'})}</span>
                        ...
                        {material.pieces !== undefined && (
                            <div className="bg-slate-50/60 rounded-md p-3 border border-slate-200">
                                <span className="text-[10px] font-medium text-slate-500 uppercase tracking-wider block mb-1">{tx(lang, {fr:'Pièces concernées', ar:'القطع المعنية', en:'Pieces concerned', es:'Piezas concernidas', pt:'Peças concernidas', tr:'İlgili parçalar'})}</span>
                    ...
                    <div className="rounded-md border border-slate-200 p-3 sm:p-4">
                        <div className="flex items-center gap-2 mb-2.5">
                            ...
                            <span className="text-[11px] font-medium text-slate-500 uppercase tracking-wider">{tx(lang, {fr:'Fournisseur & Livraison', ar:'المورد والتسليم', en:'Supplier & Delivery', es:'Proveedor y Entrega', pt:'Fornecedor e Entrega', tr:'Tedarikçi ve Teslimat'})}</span>
                        </div>
                        {fournisseurNom ? (
                            <div className="space-y-2">
                                ...
                                {delaiLivraison !== null && (
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-[12px] text-slate-600">{tx(lang, {fr:'Délai de livraison :', ar:'مدة التسليم :', en:'Delivery time:', es:'Plazo de entrega:', pt:'Prazo de entrega:', tr:'Teslim süresi:'})} <span className="font-semibold">{delaiLivraison} {tx(lang, {fr:'jours', ar:'أيام', en:'days', es:'días', pt:'dias', tr:'gün'})}</span></span>
                                    </div>
                                )}
                            </div>
                        ) : (
                            <p className="text-[12px] text-slate-400 italic">{tx(lang, {fr:'Aucun fournisseur renseigné', ar:'لا يوجد مورد مدخل', en:'No supplier entered', es:'Ningún proveedor informado', pt:'Nenhum fornecedor informado', tr:'Tedarikçi girilmemiş'})}</p>
                        )}
                    </div>
                    ...
                    <div className="rounded-md border border-slate-200 p-3 sm:p-4">
                        <div className="flex items-center gap-2 mb-3">
                            ...
                            <span className="text-[11px] font-medium text-slate-500 uppercase tracking-wider">{tx(lang, {fr:'État du Stock', ar:'حالة المخزون', en:'Stock Status', es:'Estado del Stock', pt:'Estado do Stock', tr:'Stok Durumu'})}</span>
                        </div>
                        <div className="grid grid-cols-3 gap-3 text-center">
                            <div>
                                <span className="text-[10px] text-slate-400 font-medium uppercase block mb-0.5">{tx(lang, {fr:'En Stock', ar:'في المخزون', en:'In Stock', es:'En Stock', pt:'Em Stock', tr:'Stokta'})}</span>
                                <span className={`text-[14px] font-semibold tabular-nums ${stockActuel > 0 ? 'text-emerald-700' : 'text-slate-400'}`}>{fmt(stockActuel)}</span>
                            </div>
                            <div>
                                <span className="text-[10px] text-slate-400 font-medium uppercase block mb-0.5">{tx(lang, {fr:'Besoin', ar:'الاحتياج', en:'Need', es:'Necesidad', pt:'Necessidade', tr:'İhtiyaç'})}</span>
                                <span className="text-[14px] font-semibold text-slate-900 tabular-nums">{fmt(material.qtyToBuy)}</span>
                            </div>
                            <div>
                                <span className="text-[10px] text-slate-400 font-medium uppercase block mb-0.5">{tx(lang, {fr:'Manque', ar:'الناقص', en:'Shortage', es:'Falta', pt:'Falta', tr:'Eksik'})}</span>
