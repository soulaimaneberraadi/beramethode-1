...
export default function VentesDashboard({ lang, currency = 'MAD', detail: detailControlled, onDetailChange, onOuvrirFicheClient }: Props) {
    ...
    /** Les textes de la repartition, passes au composant partage : lui ne
     *  connait pas la langue de la page. */
    const textesRepartition = {
        rien: T.rien, sansPrix: T.sansPrix, sansPrixCourt: T.sansPrixCourt,
        majoriteNonRenseigne: T.majoriteNonRenseigne,
    };

    const Repartition = ({ titre, lignes, unite, axe }: {
        titre: string;
        lignes: Array<{ cle: string; ca: number; valeur?: number; poids?: number; detail: string; vignette?: React.ReactNode; sansPrix?: boolean; titre?: string }>;
        unite?: string;
        axe?: AxeOuvert['axe'];
    }) => (
        <RepartitionUI
            ...
            libelle={libelle}
            onOuvrir={axe ? (l => setAxeOuvert({ axe, valeur: l.cle, titre: l.titre })) : undefined}
        />
    );

    const TableauJours = ({ colonnes, lignes }: { colonnes: string[]; lignes: Array<{ jour: string; cellules: string[]; alerte?: string }> }) => (
        <TableauJoursUI colonnes={colonnes} lignes={lignes}
            textes={{ titre: T.jourParJour, jour: T.jourColonne, rien: T.rien }} />
    );
                ...
                <>
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2.5">
                        <Tuile titre={T.ca} valeur={`${nf(data.kpis.ca)} ${currency}`} icone={<TrendingUp className="w-3.5 h-3.5" />} onOuvrir={() => setDetail('ca')}
                            delta={<Delta actuel={data.kpis.ca} avant={data.precedent.ca} titre={T.periodePrecedente} />} />
                        <Tuile titre={T.pieces} valeur={nf(data.kpis.pieces)} icone={<Boxes className="w-3.5 h-3.5" />} onOuvrir={() => setDetail('pieces')}
                            delta={<Delta actuel={data.kpis.pieces} avant={data.precedent.pieces} titre={T.periodePrecedente} />} />
                        <Tuile titre={T.tickets} valeur={nf(data.kpis.tickets)} icone={<Receipt className="w-3.5 h-3.5" />} onOuvrir={() => setDetail('tickets')}
                            delta={<Delta actuel={data.kpis.tickets} avant={data.precedent.tickets} titre={T.periodePrecedente} />} />
                                ...
                                {serieComplete.length === 0 ? (
                                    ...
                                    <>
                                        ...
                                        {seriesVisibles.length === 0 ? (
                                            ...
                                            <Courbes jours={serieComplete.map(j => j.jour)} series={seriesVisibles} cumul={cumule} devise={currency} libelle={libelle} />
                                        )}
