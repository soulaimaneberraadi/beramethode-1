    ...
    return (
        ...
        >
            <div className="px-5 py-4 bg-slate-50 dark:bg-dk-bg border-b border-slate-100 dark:border-dk-border flex items-center gap-2">
                <KeyRound className="w-5 h-5 text-slate-500 dark:text-dk-muted" />
                <h2 className="font-bold text-slate-800 dark:text-dk-text">{t.title}</h2>
            </div>

            <div className="p-5 space-y-4">
                <p className="text-xs text-slate-500 dark:text-dk-muted font-medium">{t.desc}</p>
                ...
                <div className="flex flex-col sm:flex-row gap-3 items-stretch">
                    <input
                        ...
                        className="flex-1 bg-slate-50 dark:bg-dk-bg border-2 border-slate-200 dark:border-dk-border rounded-xl px-4 py-3 outline-none focus:border-indigo-500 font-mono font-bold tracking-wider text-slate-800 dark:text-dk-text transition-all text-center sm:text-start uppercase placeholder:text-slate-300 placeholder:tracking-normal"
                    />
                    <button
                        type="button"
                ...
                {license.ok && (
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
                        ...
                        <div className="bg-slate-50 dark:bg-dk-bg border border-slate-200 dark:border-dk-border rounded-xl p-3 text-center">
                            <span className="block text-[10px] font-bold uppercase tracking-wide text-slate-400 dark:text-dk-muted mb-1">{t.expires}</span>
                            <span className="text-xs font-black text-slate-700 dark:text-dk-text-soft font-mono">{fmtDate(license.expires_at)}</span>
                        </div>
                        <div className="bg-slate-50 dark:bg-dk-bg border border-slate-200 dark:border-dk-border rounded-xl p-3 text-center">
                            <span className="block text-[10px] font-bold uppercase tracking-wide text-slate-400 dark:text-dk-muted mb-1">{t.daysLeft}</span>
                            <span className="text-sm font-black text-slate-800 dark:text-dk-text">{license.daysLeft}</span>
                        </div>
                        <div className="bg-slate-50 dark:bg-dk-bg border border-slate-200 dark:border-dk-border rounded-xl p-3 text-center">
                            <span className="block text-[10px] font-bold uppercase tracking-wide text-slate-400 dark:text-dk-muted mb-1">{t.modulesCount}</span>
                            <span className="text-sm font-black text-slate-800 dark:text-dk-text">{license.modules.length}</span>
                        </div>
                    </div>
                )}
            </div>
