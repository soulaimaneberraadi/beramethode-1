...
- تحدّث مع المستخدم (Soulaimane Berraadi) **باللغة العربية الفصحى (Fosha) فقط**.
- **تجنّب استعمال الدارجة المغربية أو اللغات الأجنبية الأخرى** إلا إذا طلب المستخدم ذلك صراحة.
- المصطلحات التقنية (API, JWT, SQLite, HMAC, RLS) تبقى بالإنجليزية داخل النص باللغة العربية.
...
5. **الهدف الافتراضي هو Vercel، لا المحلي:** العمل الحقيقي يجري على النشر السحابي (`VITE_STATIC_MODE=true` + Supabase). المحلي (البورت 7000 + SQLite) للتجريب والتطوير فقط. أي إصلاح يجب أن يعمل **في الوضعين**، وأن يُدمج في `master` ليُنشر على Vercel — لا يكفي أن ينجح محلياً.
6. **البيانات الأوتوماتيكية:** فلو البيانات الأوتوماتيكي يجب أن يكون شغّال بشكل إجباري. تأكد من أن جميع البيانات تتدفق بشكل آلي بين الخادم والواجهة دون تدخل يدوي.
