...
export default function App() {
    ...
    useEffect(() => {
        ...
        if (ric) {
            const id = ric(() => { preloadAllChunks(); }, { timeout: 8000 });
                    ...
                    {currentView === 'suivi' && (
                        <div className="flex min-h-0 w-full flex-1 flex-col overflow-hidden">
                            <SuiviProduction
                                ...
                                onRemovePoste={async (modelId, posteId) => {
                                    /* Meme precaution que l'ajout : on relit la version fraiche
                                       avant d'ecrire, car `POST /api/models` remplace le modele
                                       entier. Les releves de poste_suivi ne sont pas touches —
                                       la gamme perd une ligne, l'historique garde la sienne. */
                                    const local = models.find(x => x.id === modelId);
                                    if (!local) return;
                                    let base = local;
                                    if (!IS_STATIC) {
                                ...
                                onSetPosteTemps={async (modelId, posteId, tempsMin) => {
                                    /* Le temps standard d'un poste du releve. Il vit dans
                                       `suiviPostes`, jamais dans la gamme : la gamme chiffre le
                                       prix de revient, et un temps decide a l'atelier n'a pas a
                                       deplacer un cout tout seul. */
                                    const local = models.find(x => x.id === modelId);
                                    if (!local) return;
                                    let base = local;
                                    if (!IS_STATIC) {
